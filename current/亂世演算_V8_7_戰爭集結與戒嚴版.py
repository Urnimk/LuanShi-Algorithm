...
ENGINE_VERSION = "亂世演算V8_7_戰爭集結與戒嚴版"
...
REBUILD_PROTECTION_BLOCK_ATTACK = True      # 是否可打架
REBUILD_PROTECTION_BLOCK_ALLIANCE = False   # 是否可加入聯盟
...
SUBJUGATION_BEFORE_ANNEXATION = 0       # 第一次國都陷落先附庸，再次才可能滅國
...
# 【戰爭集結與預警】宣戰後先集結，不在同一年瞬間交戰。
WAR_MUSTER_YEARS_MIN = 3
WAR_MUSTER_YEARS_MAX = 5

# 【國家生存戒嚴】低軍備自動觸發，採進入／解除雙門檻避免反覆開關。
...
# 【本屆後段爭冠】前五名每年取得決策機會，但仍由 Q 值選擇行動。
LATE_SEASON_CONTENDER_COUNT = 5

# 【復名政變】曾被政變改國號的國家，後續政變有機會恢復原始國名。
RESTORATION_COUP_CHANCE = 0.25
...
UNDEFENDED_MAX_SIEGE_YEARS = 10        # 無守軍但仍有城防，最多再抵抗 N 年
...
def _ensure_dynamic_country_state(country_key, data):
    ...
    data.setdefault("original_name_restorations", 0)
    data.setdefault("martial_law_active", False)
    data.setdefault("martial_law_since_turn", 0)
...
def _update_strategic_mode(country_key, data, countries, rank_map, total_world_power):
    ...
    if data.get("martial_law_active", False):
        mode = "全國戒嚴"
    elif food_turns < 2.2 or int(data.get("famine_streak", 0) or 0) > 0:
        mode = "糧荒求生"
    elif exhaustion >= 68:
        mode = "戰後復甦"
...
def _apply_regime_change(
    ...
    if kind == "RESTORATION":
        # country_key 是最初建立國家時的原始國名；政變改名後仍永久保留。
        new_name = str(country_key)
        new_regime_type = "復名政權"
        data["political_stability"] = 52.0
        data["infamy"] = max(0, int(data.get("infamy", 0) or 0) - 8)
        data["military_target_ratio"] = float(
            _clamp(float(data.get("military_target_ratio", 0.30)), 0.22, 0.40)
        )
        data["original_name_restorations"] = max(
            0, int(data.get("original_name_restorations", 0) or 0)
        ) + 1
        leave_chance = 0.08
        kind_name = "復名政變"
...
def _maybe_trigger_regime_change(
    ...
    for key in shuffled:
        ...
        current_root = str(data.get("regime_name_root", key) or key).strip()
        if current_root != str(key).strip() and random.random() < RESTORATION_COUP_CHANCE:
            kind = "RESTORATION"
        elif military_ratio >= 0.36 and random.random() < 0.62:
            kind = "MILITARY"
...
def update_rts_economy_and_jobs(data, market=None):
    ...
    # 士兵低於 5% 已屬亡國級危機，不能等待稀疏的 Q 決策碰巧選到動員。
    # 戒嚴只提高徵兵速度，仍受農民、糧食、金屬與 50% 軍民上限約束。
    soldier_ratio_at_start = soldiers / max(1, pop_total)
    martial_law_active = bool(data.get("martial_law_active", False))
    if not martial_law_active and soldier_ratio_at_start < MARTIAL_LAW_TRIGGER_SOLDIER_RATIO:
        martial_law_active = True
        data["martial_law_active"] = True
        data["martial_law_since_turn"] = int(data.get("current_turn", 0) or 0)
        data["last_event"] = "士兵比例低於5%，發布全國戒嚴與緊急徵召令"
        print(
            f"🚨 [全國戒嚴]【{c_name}】士兵僅占 {soldier_ratio_at_start:.1%}；"
            f"徵兵能力 ×{MARTIAL_LAW_RECRUIT_MULTIPLIER:.0f}，軍民維持消耗 ×{MARTIAL_LAW_UPKEEP_MULTIPLIER:.0f}，"
            f"直到士兵比例恢復 {MARTIAL_LAW_RELEASE_SOLDIER_RATIO:.0%}。"
        )
    elif martial_law_active and soldier_ratio_at_start >= MARTIAL_LAW_RELEASE_SOLDIER_RATIO:
        martial_law_active = False
        data["martial_law_active"] = False
        data["last_event"] = "士兵比例恢復30%，解除全國戒嚴"
        print(f"🕊️ [解除戒嚴]【{c_name}】士兵比例已恢復 {soldier_ratio_at_start:.1%}。")
    upkeep_mult = MARTIAL_LAW_UPKEEP_MULTIPLIER if martial_law_active else 1.0
    ...
    if is_famine_warning and not martial_law_active:
    ...
    if martial_law_active:
        effective_ratio = max(
            effective_ratio,
            MARTIAL_LAW_RELEASE_SOLDIER_RATIO,
        )
    elif is_famine_warning:
    ...
    if soldiers > desired_soldiers and not is_famine_warning and not martial_law_active:
    ...
    elif soldiers < desired_soldiers and (not is_famine_warning or martial_law_active):
        ...
        if martial_law_active:
            recruit_roll = int(
                recruit_roll * MARTIAL_LAW_RECRUIT_MULTIPLIER
            )
    ...
    metal_maintenance = int(
        math.ceil(soldiers * BALANCE["SOLDIER_METAL_UPKEEP"] * upkeep_mult)
    )
    ...
    data["martial_law_active"] = bool(martial_law_active)
...
class CountryRLAgent:
    ...
    def _get_strategic_state(self, country_data, total_world_power, max_power, alive_count):
        ...
        contender = (
            "TOP3" if rank <= 3
            else "TOP5" if rank <= 5 and phase == "LATE"
            else "TOP10" if rank <= 10
            else "FIELD"
        )
        existing_state = (
...
def _finalize_muster_declaration_learning(
    agent,
    attacker_key,
    c_data,
    before_data,
    ...
    """宣戰進入集結期時立即完成本次 RL 記帳，戰果則留給延遲 Reward。"""
    c_data["power"] = calculate_rts_power(c_data)
    enforce_farmer_majority(c_data)
    ...
    before_data["current_war_count"] = int(current_war_count)
    after_data = _rl_snapshot(c_data)
    reward = agent.calculate_reward(
        before_data,
        after_data,
    ...
    c_data["last_action"] = RL_ACTIONS[0]
    c_data["last_reward"] = round(reward, 2)
    ...
    agent.schedule_delayed_evaluation(before_data, 0, turn_counter, True)
    return reward
...
def resolve_persistent_war_round(
    ...
    # 宣戰與正式交鋒之間保留 3~5 年集結期。防守方已知情，可在這段時間
    # 透過正常年度經濟、AI決策與戒嚴機制補兵、築城或調整產業。
    combat_starts_turn = int(war.get("combat_starts_turn", war.get("started_turn", 0)) or 0)
    if int(turn_counter) < combat_starts_turn:
        war["phase"] = "MUSTERING"
        war["last_turn"] = int(turn_counter)
        war["last_tactic"] = "集結準備"
        return {
            "ended": False,
            "winner": None,
            "reason": "雙方集結中",
            "mustering": True,
            "years_remaining": combat_starts_turn - int(turn_counter),
        }
    if war.get("phase") == "MUSTERING":
        war["phase"] = "FIGHTING"
        print(
            f"⚔️ [集結完成] {war_id}：【{war.get('attacker_name', '進攻方')}】"
            f"開始進攻【{war.get('defender_name', '防守方')}】。"
        )
...
def _war_refresh_country_status(countries, active_wars, turn_counter=0):
    ...
    for war_id, war in active_wars.items():
        ...
        combat_starts_turn = int(war.get("combat_starts_turn", 0) or 0)
        mustering = combat_starts_turn > int(turn_counter)
        gather_left = max(0, combat_starts_turn - int(turn_counter))
        for m in _war_live_members(war.get("attacker_members"), countries):
            countries[m]["current_wars"].append(war_id)
            countries[m]["war_status"] = (
                f"集結進攻【{war.get('defender_name', '敵國')}】（尚餘 {gather_left} 年）"
                if mustering else
                f"進攻【{war.get('defender_name', '敵國')}】（第 {rounds}/{duration} 回合）"
            )
        ...
        for m in _war_live_members(war.get("defender_members"), countries):
            countries[m]["current_wars"].append(war_id)
            countries[m]["war_status"] = (
                f"敵軍集結【{war.get('attacker_name', '敵國')}】（{gather_left} 年後進攻）"
                if mustering else
                f"防禦【{war.get('attacker_name', '敵國')}】（第 {rounds}/{duration} 回合）"
            )
            countries[m]["war_log_terms"] = list(dict.fromkeys(
                countries[m].get("war_log_terms", [])
                + [war.get("attacker_name", ""), war.get("defender_name", ""), war_id]
...
def thread_stat_monitor():
    ...
    while True:
        ...
        for rank, country_key in enumerate(alive_sorted, 1):
            ...
            countries[country_key]["top_five_contender"] = rank <= LATE_SEASON_CONTENDER_COUNT
            countries[country_key]["championship_drive"] = (
                "爭奪冠軍"
                if season_phase == "LATE" and rank <= LATE_SEASON_CONTENDER_COUNT
                else "一般競爭"
            )
        ...
        total_world_population = sum(
            v["pop_total"] for v in countries.values() if v.get("is_alive", True)
        )
        ...
        if len(alive_sorted) >= 2:
            ...
            # 前／中段仍提高前三名的行動機會；後段前五名每年都有決策機會。
            # 實際做什麼仍由「爭冠」狀態的 Q 值決定，不強迫宣戰。
            if season_phase == "LATE":
                for contender in alive_sorted[:LATE_SEASON_CONTENDER_COUNT]:
                    if contender not in active_countries:
                        active_countries.append(contender)
            else:
                contender_chance = {"EARLY": 0.35, "MIDDLE": 0.60}[season_phase]
                ...
                if action == 0:
                    ...
                    if possible_defenders:
                        ...
                        if existing_conflict is None:
                            ...
                            muster_years = random.randint(
                                WAR_MUSTER_YEARS_MIN,
                                WAR_MUSTER_YEARS_MAX,
                            )
                            active_wars[war_id] = {
                                ...
                                "phase": "MUSTERING",
                                "muster_years": muster_years,
                                "combat_starts_turn": turn_counter + muster_years,
                            ...
                            print(
                                f"📯 [集結宣戰] {war_id}：【{att_entity['name']}】向【{def_entity['name']}】宣戰！"
                                f"雙方將集結 {muster_years} 年，第 {turn_counter + muster_years} 年正式交鋒；"
                                f"戰事預定持續 {active_wars[war_id]['planned_duration']} 回合。"
                                f"{front_note}"
                            )
                            for defender_member in active_wars[war_id]["defender_members"]:
                                if defender_member not in countries:
                                    continue
                                countries[defender_member]["last_event"] = (
                                    f"收到戰爭警報：{att_entity['name']}正在集結，"
                                    f"預計 {muster_years} 年後進攻"
                                )
                                countries[defender_member]["war_status"] = (
                                    f"敵軍集結【{att_entity['name']}】（{muster_years} 年後進攻）"
                                )

                        # 新戰爭在集結期不發生即時傷亡或出征消耗；本次宣戰決策先記帳，
                        # 真正勝負由 3~5 年後的持續戰爭及延遲 Reward 回饋。
                        if created_new_war:
                            _finalize_muster_declaration_learning(
                                agent,
                                attacker_key,
                                c_data,
                                before_data,
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # ----------------------------------------------------
                    ...
                    print(
                        ...
                        f"持續 {INDUSTRY_FOCUS_DURATION} 年。"
                    )
        ...
        season_elapsed_years = max(0, int(turn_counter) - int(season_start_turn))
        season_remaining_years = max(0, YEARS_PER_SEASON - season_elapsed_years)
        rank_text = f"亂世演算 第 {count_round} 屆 戰力大戰 ===== 世界曆 {count_round*YEARS_PER_SEASON-season_remaining_years} 年 ===== 剩餘 {season_remaining_years} 年結束\n"
        rank_text += (
            f"全陸總戰力 = {total_world_power} ({world_stage_g})"
            f" | 世界人口 = {total_world_population}"
