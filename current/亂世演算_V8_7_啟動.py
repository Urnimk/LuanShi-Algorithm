...
try:
    from 亂世演算_V8_7_戰爭集結與戒嚴版 import (
        thread_stat_monitor,
    ...
    MARKET_ECONOMY_ENABLED = engine.MARKET_ECONOMY_ENABLED
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V8_7｜戰爭集結與戒嚴版"
...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        columns_country = [
            ...
            ("pop", "總人口"), ("gold", "大洋" if MARKET_ECONOMY_ENABLED else "貨幣關閉"), ("championship_count", "冠軍數"), ("status", "狀態")    
        ]
    ...
    def resolve_country_key(self, name):
        ...
        name = str(name).strip()
        # UI 會用符號標示宗主與附庸；導航時移除純顯示符號。
        name = name.removeprefix("◇ ").removeprefix("♛ ").strip()
        if "〔" in name:
    ...
    def _is_active_vassal(self, data):
        """目前仍在附庸期限內，而且宗主國欄位有效。"""
        return bool(
            str(data.get("protector_country", "") or "").strip()
            and int(data.get("vassal_until_turn", 0) or 0) > self.raw_turn_counter
            and data.get("is_alive", True)
        )

    def _active_vassals_of(self, country_key):
        """取得指定宗主國目前仍有效的附庸國 key。"""
        return [
            key
            for key, data in self.raw_countries_data.items()
            if self._is_active_vassal(data)
            and str(data.get("protector_country", "") or "") == country_key
        ]

    def _relation_display_name(self, country_key, data):
        display_name = str(data.get("display_name", country_key))
        if self._is_active_vassal(data):
            return f"◇ {display_name}"
        if self._active_vassals_of(country_key):
            return f"♛ {display_name}"
        return display_name

    def _relation_status(self, country_key, data, base_status="存活"):
        if self._is_active_vassal(data):
            master_key = str(data.get("protector_country", "") or "")
            master = self.raw_countries_data.get(master_key, {})
            master_name = master.get("display_name", master_key)
            remaining = max(
                0,
                int(data.get("vassal_until_turn", 0) or 0) - self.raw_turn_counter,
            )
            return f"附庸→{master_name}（{remaining}年）"
        vassals = self._active_vassals_of(country_key)
        if vassals:
            return f"宗主｜附庸{len(vassals)}國"
        return base_status
    ...
    def on_alliance_selected(self, event=None):
        ...
        for member in members:
            ...
            display_name = self._relation_display_name(key, c)
    ...
    def on_chronicle_country_selected(self, event=None):
        ...
        if signature != self._chronicle_signature:
            ...
            if children:
                self.chronicle_tree.see(children[-1]) # 初始世代保持可見。
                self.chronicle_tree.see(children[0]) # 最新世代保持可見。
    ...
    def on_country_selected(self, event=None):
        ...
        relation_rows = []
        if self._is_active_vassal(c):
            master_key = str(c.get("protector_country", "") or "")
            master = self.raw_countries_data.get(master_key, {})
            master_name = master.get("display_name", master_key)
            master_alliance = master.get("current_alliance", "") or "無聯盟"
            remaining = max(
                0,
                int(c.get("vassal_until_turn", 0) or 0) - self.raw_turn_counter,
            )
            relation_rows.extend([
                ("附庸關係", f"◇ 附庸國｜宗主：{master_name}｜尚餘 {remaining:,} 年"),
                ("宗主聯盟", master_alliance),
            ])
        else:
            vassal_keys = self._active_vassals_of(selected_name)
            if vassal_keys:
                vassal_names = [
                    self.raw_countries_data[key].get("display_name", key)
                    for key in vassal_keys
                ]
                relation_rows.extend([
                    ("附庸關係", f"♛ 宗主國｜附庸 {len(vassal_names)} 國"),
                    ("附庸名單", "、".join(vassal_names)),
                ])
        ...
        basic_rows = [
            (
                "國家狀態",
                (
                    # (f"🟢存活 (淪陷:{c.get('subjugation_count',0)})" if c.get("is_alive", True) else "🔴 滅亡")
                    (f"🟢存活 " if c.get("is_alive", True) else "🔴 滅亡")
            ...
            *relation_rows,
            ...
            (
                "重建保護",
                (
                    f"尚餘 {max(0, max(int(c.get('rebuild_protection_until_turn', 0) or 0), int(c.get('newbie_protection_until_turn', 0) or 0)) - self.raw_turn_counter):,} 年"
                    f"｜禁止攻擊、聯盟"
                    if max(
                        int(c.get("rebuild_protection_until_turn", 0) or 0),
                        int(c.get("newbie_protection_until_turn", 0) or 0)
                        if c.get("regime_type") == "重建政權" else 0,
                    ) > self.raw_turn_counter
                    else "無"
                ),
            ),
        ...
        econ_rows = [
            ...
            ((f"糧食 / {self.raw_world_bank.get('currency', '大洋')}" if MARKET_ECONOMY_ENABLED else "糧食 / 無貨幣"),
             (f"{c.get('food', 0):,} / {c.get('gold', 0):,}" if MARKET_ECONOMY_ENABLED else f"{c.get('food', 0):,} / —")),
        ...
        for idx, record in enumerate(
            ...
            if match:
                record_text = (
                    f"{match.group(1)}屆({int(match.group(1))*500} 年)｜{int(match.group(2))/10000:.1f} 萬｜{match.group(3)}"
                )
    ...
    def refresh_world_market_panel(self):
        ...
        if not MARKET_ECONOMY_ENABLED:
            self.tree_country.heading("gold", text="貨幣關閉")
            self.market_order_tree.heading("price", text="單價(停用)")
            self.market_bank_label.configure(text="🚫 無貨幣模式｜世界貨幣、銀行、市場撮合與交易 AI 均已關閉")
            self.market_stats_label.configure(text="政策成本：各國當下糧食、木材、金屬各 10%｜既有金幣資料不參與決策")
            self._replace_tree_rows(self.market_quote_tree, [])
            self._replace_tree_rows(self.market_order_tree, [])
            self._replace_tree_rows(self.market_trade_tree, [])
            return
    ...
    def update_ui_data(self):
        ...
        try:
            ...
            if os.path.exists(save_path("war_live_countries.json")):
                ...
                country_rows = []
                for key, data in self.raw_countries_data.items():
                    disp_name = self._relation_display_name(key, data)
                    gen = data.get(
                        "country_generation",
                        data.get("gen", 1),
                    )
                    ...
                    gold = data.get("gold", 0) if MARKET_ECONOMY_ENABLED else "—"
                    ...
                    if not data.get("is_alive", True):
                        ...
                        status = "存活"
                    status = self._relation_status(key, data, status)
                    country_rows.append(
            ...
            if os.path.exists(save_path("war_live_alliances.json")):
                ...
                for ally_name, details in self.raw_alliances_data.items():
                    ...
                    m_count = (
                        ...
                        f"{details.get('member_count', 0)}｜5"
                    )
