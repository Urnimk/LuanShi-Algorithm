...
ENGINE_VERSION = "亂世演算V8_4_1_多層強化學習實驗版"
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # Action 3: 養精蓄銳 (發展經濟)
                    ...
                    c_data["military_target_ratio"] = float(
                        _clamp(c_data.get("military_target_ratio", 0.30) - 0.008, 0.12, 0.50)
                    )
                ...
                # ----------------------------------------------------
                    ...
                    c_data["military_target_ratio"] = float(
                        _clamp(c_data.get("military_target_ratio", 0.30) - 0.006, 0.12, 0.50)
                    )
                    ...
                    investment = min(max(0, int(c_data.get("wood", 0) or 0)), int(food_gain * 1.5))
                    food_gain = min(food_gain, int(investment / 1.5)) if investment > 0 else 0
                    c_data["wood"] -= investment
                    c_data["food"] += food_gain
                    action_success = food_gain > 0
