...
try:
    from 亂世演算_V8_4_2_多經濟資產分級 import (
        thread_stat_monitor,
    ...
    pattern = os.path.join(
        ...
        "亂世演算_V8_4_1_多層強化學習實驗版*.py",
    )
    candidates = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    if not candidates:
        raise
    spec = importlib.util.spec_from_file_location("亂世演算_v841_engine", candidates[0])
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V8_4_1｜多層 Expected SARSA(λ) 實驗版"
...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        columns_country = [
            ...
            ("pop", "總人口"), ("gold", "金幣"), ("championship_count", "冠軍數"), ("status", "狀態")    
        ]
    ...
    def refresh_world_market_panel(self):
        ...
        self.market_bank_label.configure(
            text=(
                f"🏦 世界銀行｜貨幣：{bank.get('currency', '金幣')}｜"
