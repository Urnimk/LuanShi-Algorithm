...
from 亂世演算_V8_2_長週期市場版 import thread_stat_monitor
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V8_2｜自主學習 AI・長週期世界市場・貨幣守恆・編年史"
...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        # 5. 💱 世界報價系統 Tab（位於「聯盟詳細查詢」右側）
        self.tab_market = ttk.Frame(self.left_notebook)
        self.left_notebook.add(self.tab_market, text=" 💱 世界報價系統 ")

        self.market_summary_group = tk.LabelFrame(
            self.tab_market, text=" 🏦 世界銀行・市場摘要 ",
            bg=COLOR_BG_MAIN, fg=COLOR_FG_ACCENT, font=FONT_UI_BOLD
        )
        self.market_summary_group.pack(fill="x", padx=8, pady=(8, 4))
        self.market_summary_tree = ttk.Treeview(
            self.market_summary_group, columns=("item", "value"), show="headings", height=12
        )
        self.market_summary_tree.heading("item", text="項目")
        self.market_summary_tree.heading("value", text="內容")
        self.market_summary_tree.column("item", width=180, anchor="w", stretch=False)
        self.market_summary_tree.column("value", width=520, anchor="w", stretch=True)
        self.market_summary_tree.pack(fill="x", expand=True, padx=4, pady=4)

        self.market_price_group = tk.LabelFrame(
            self.tab_market, text=" 📊 三大資源即時報價（金幣 / 單位） ",
            bg=COLOR_BG_MAIN, fg=COLOR_FG_ACCENT, font=FONT_UI_BOLD
        )
        self.market_price_group.pack(fill="x", padx=8, pady=4)
        self.market_price_tree = ttk.Treeview(
            self.market_price_group,
            columns=("resource", "reference", "last", "bid", "ask", "trend", "volume", "orders"),
            show="headings", height=4,
        )
        for col, title, width, anchor_name in (
            ("resource", "資源", 80, "center"),
            ("reference", "參考價", 85, "e"),
            ("last", "最近成交", 90, "e"),
            ("bid", "最高買價", 90, "e"),
            ("ask", "最低賣價", 90, "e"),
            ("trend", "趨勢", 70, "center"),
            ("volume", "本輪量", 75, "e"),
            ("orders", "買/賣單", 85, "center"),
        ):
            self.market_price_tree.heading(col, text=title)
            self.market_price_tree.column(col, width=width, anchor=anchor_name, stretch=(col == "resource"))
        self.market_price_tree.pack(fill="x", expand=True, padx=4, pady=4)

        self.market_book_group = tk.LabelFrame(
            self.tab_market, text=" 🧾 國家掛牌簿（最佳報價優先） ",
            bg=COLOR_BG_MAIN, fg=COLOR_FG_ACCENT, font=FONT_UI_BOLD
        )
        self.market_book_group.pack(fill="both", expand=True, padx=8, pady=(4, 8))
        self.market_book_tree = ttk.Treeview(
            self.market_book_group,
            columns=("side", "resource", "country", "price", "remaining", "expires"),
        ...
        for col, title, width, anchor_name in (
            ...
            ("price", "報價", 90, "e"),
            ("remaining", "剩餘量", 85, "e"),
            ("expires", "到期 Turn", 95, "center"),
        ):
            self.market_book_tree.heading(col, text=title)
            self.market_book_tree.column(col, width=width, anchor=anchor_name, stretch=(col == "country"))
        market_scroll = ttk.Scrollbar(self.market_book_group, orient="vertical", command=self.market_book_tree.yview)
        self.market_book_tree.configure(yscrollcommand=market_scroll.set)
        self.market_book_tree.pack(side="left", fill="both", expand=True, padx=(4, 0), pady=4)
        market_scroll.pack(side="right", fill="y", padx=(0, 4), pady=4)
        ...
        self.raw_world_bank = {}
        self.raw_world_market = {}
        self.raw_market_orders = []
        self.raw_world_mode = {
            "code": "NORMAL",
            "name": "平常局勢",
            "remaining_turns": 0,
    ...
    def on_country_selected(self, event=None):
        ...
        econ_rows = [
            ...
            ("糧食 / 金幣", f"{c.get('food', 0):,} / {c.get('gold', 0):,}"),
            ("木材 / 金屬", f"{c.get('wood', 0):,} / {c.get('metal', 0):,}"),
            # (
            #     "單筆掛單上限",
            #     f"{int(c.get('market_order_cap', 0) or 0):,} 單位",
            # ),
            ("最近市場掛牌", str(c.get('last_market_order', '無'))),
        ]
    ...
    def update_market_view(self):
        ...
        resources = market.get("resources", {}) or {}
        orders = self.raw_market_orders or []

        circulating = sum(
            max(0, int(c.get("gold", 0) or 0))
            for c in self.raw_countries_data.values()
        )
        issued = max(0, int(bank.get("issued_total", 0) or 0))
        reserve = max(0, int(bank.get("reserve_gold", 0) or 0))
        total_accounted = circulating + reserve
        circulation_ratio = (circulating / issued * 100.0) if issued > 0 else 0.0
        conservation_gap = issued - total_accounted
        summary_rows = [
            ("貨幣發行機構", bank.get("name", "世界銀行")),
            ("累計發行量", f"{issued:,} 金幣"),
            ("各國流通量", f"{circulating:,} 金幣"),
            ("流通率", f"{circulation_ratio:.1f}%（目標約 82%）"),
            (
                "銀行回灌方式",
                "低流動性 60% 國家｜按人口規模加權",
            ),
            (
                "單筆掛單倍率",
                "100人=180；其後約人口×1.8（最高1,000,000）",
            ),
            ("世界銀行準備金", f"{reserve:,} 金幣"),
            ("政策累計回收", f"{int(bank.get('policy_collected', 0) or 0):,} 金幣"),
            ("累計流動性回灌", f"{int(bank.get('liquidity_released', bank.get('dividends_paid', 0)) or 0):,} 金幣"),
            ("貨幣守恆差額", f"{conservation_gap:+,} 金幣"),
            ("舊版找回金幣", f"{int(bank.get('audit_recovered_gold', 0) or 0):,} 金幣"),
            ("市場撮合 Turn", f"{int(market.get('last_clearing_turn', 0) or 0):,}"),
        ]
        self._replace_tree_rows(self.market_summary_tree, summary_rows)

        name_map = {"food": "糧食", "wood": "木材", "metal": "金屬"}
        price_rows = []
        for res in ("food", "wood", "metal"):
            d = resources.get(res, {}) or {}
            bid = d.get("best_bid")
            ask = d.get("best_ask")
            price_rows.append((
                name_map[res],
                f"{float(d.get('reference_price', 0) or 0):.2f}",
                f"{float(d.get('last_price', 0) or 0):.2f}",
                "-" if bid is None else f"{float(bid):.2f}",
                "-" if ask is None else f"{float(ask):.2f}",
                str(d.get("trend", "持平")),
                f"{int(d.get('turn_volume', 0) or 0):,}",
                f"{int(d.get('open_buy_orders', 0) or 0)} / {int(d.get('open_sell_orders', 0) or 0)}",
            ))
        self._replace_tree_rows(self.market_price_tree, price_rows)

        def order_sort_key(o):
            side = str(o.get("side", ""))
            price = float(o.get("price", 0) or 0)
            return (str(o.get("resource", "")), 0 if side == "BUY" else 1, -price if side == "BUY" else price, int(o.get("created_turn", 0) or 0))

        book_rows = []
        for o in sorted([x for x in orders if isinstance(x, dict)], key=order_sort_key)[:100]:
            book_rows.append((
                "買" if o.get("side") == "BUY" else "賣",
                name_map.get(o.get("resource"), str(o.get("resource", ""))),
                o.get("country_name") or o.get("country", ""),
                f"{float(o.get('price', 0) or 0):.2f}",
                f"{int(o.get('remaining', 0) or 0):,}",
                f"{int(o.get('expires_turn', 0) or 0):,}",
            ))
        if not book_rows:
            book_rows = [("-", "-", "目前沒有掛牌", "-", "-", "-")]
        self._replace_tree_rows(self.market_book_tree, book_rows)
    ...
    def update_ui_data(self):
        ...
        try:
            ...
            if os.path.exists("war_live_alliances.json"):
                ...
                self.raw_world_bank = a_json.get("world_bank", {}) or {}
                self.raw_world_market = a_json.get("world_market", {}) or {}
                self.raw_market_orders = a_json.get("market_orders", []) or []
