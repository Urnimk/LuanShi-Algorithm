...
SUCCESSION_PERSONALITY_CHANGE_CHANCE = 0.5 # 交接後變換性格
...
PERSONALITIES = {
    ...
    "WARMONGER": {  # 好戰狂魔：高攻擊、高背叛
        "name": "好戰狂魔",
        "atk_pref": 0.4,
        "betray_pref": 0.2,
    },

    "PACIFIST": {  # 和平主義：極少主動戰爭，幾乎不背叛
        "name": "和平主義",
        "atk_pref": 0.05,
        "betray_pref": 0.0,
    },

    "OPPORTUNIST": {  # 投機政客：等待弱點，高度背叛
        "name": "投機政客",
        "atk_pref": 0.2,
        "betray_pref": 0.3,
    },

    "EXPANSIONIST": {  # 擴張主義：積極侵略，追求領土與實力
        "name": "擴張主義",
        "atk_pref": 0.35,
        "betray_pref": 0.1,
    },

    "DEFENSIVE": {  # 防禦派：很少主動開戰，但遭攻擊後強烈反擊
        "name": "防禦派",
        "atk_pref": 0.08,
        "betray_pref": 0.02,
    },

    "TREACHEROUS": {  # 陰謀家：平時偽裝和平，關鍵時刻背刺
        "name": "陰謀家",
        "atk_pref": 0.15,
        "betray_pref": 0.45,
    },

    "HEGEMONIC": {  # 霸權派：強國特別積極，試圖壓制其他大國
        "name": "霸權派",
        "atk_pref": 0.3,
        "betray_pref": 0.15,
    },

    "BALANCER": {  # 均勢派：不希望任何國家過度強大
        "name": "均勢派",
        "atk_pref": 0.18,
        "betray_pref": 0.08,
    },

    "REVANCHIST": {  # 復仇主義：被攻擊、失去領土後會變得非常激進
        "name": "復仇主義",
        "atk_pref": 0.22,
        "betray_pref": 0.12,
    },

    "ISOLATIONIST": {  # 孤立主義：盡量不介入國際衝突
        "name": "孤立主義",
        "atk_pref": 0.03,
        "betray_pref": 0.0,
    },
...
def _apply_regime_change(
    ...
    if kind == "SUCCESSION":
        ...
        data["regime_type"] = next_regime_type
                ...
                # 正統／合法交接後，新世代有機率改變 AI 個性。
        # 只有 SUCCESSION 才會觸發，不影響其他政權更替方式。
        if random.random() < SUCCESSION_PERSONALITY_CHANGE_CHANCE:
            old_personality = data.get("personality", "OPPORTUNIST")

            personality_keys = list(PERSONALITIES.keys())

            # 排除原本人格，避免「人格變更」結果其實完全沒變。
            available_personalities = [
                key
                for key in personality_keys
                if key != old_personality
            ]

            if available_personalities:
                new_personality = random.choice(
                    available_personalities
                )

                data["personality"] = new_personality

                old_personality_name = PERSONALITIES.get(
                    old_personality,
                    {}
                ).get(
                    "name",
                ...
                new_personality_name = PERSONALITIES.get(
                    new_personality,
                    {}
                ).get(
                    "name",
                ...
                print(
                    f"🧠 【人格轉變】{old_name} → {new_name}："
                    f"新世代人格由「{old_personality_name}」"
                    f"轉變為「{new_personality_name}」。"
                )

                personality_change_text = (
                    f"；新世代人格："
                    f"{old_personality_name} → "
                    f"{new_personality_name}"
                )
            else:
                personality_change_text = ""
        else:
...
class CountryRLAgent:
    ...
    def _contextual_weights(self, state):
        ...
        personality_weights = {
            ...
            # --------------------------------------------------------
            "WARMONGER": [
                0.24, 0.045, 0.055, 0.055,
                0.085, 0.045, 0.145, 0.035,
                0.040, 0.020, 0.195, 0.035
            ],

            # --------------------------------------------------------
            # 和平主義
            # 極度避免戰爭，偏好發展、農業、貿易與防禦。
            # --------------------------------------------------------
            "PACIFIST": [
                0.010, 0.180, 0.040, 0.170,
                0.003, 0.145, 0.030, 0.180,
                0.110, 0.080, 0.010, 0.095
            ],
            ...
            # --------------------------------------------------------
            "OPPORTUNIST": [
                0.115, 0.115, 0.100, 0.105,
                0.110, 0.075, 0.080, 0.065,
                0.070, 0.040, 0.095, 0.030
            ],
            ...
            # --------------------------------------------------------
            "PREDATOR": [
                0.190, 0.055, 0.035, 0.045,
                0.085, 0.035, 0.125, 0.030,
                0.035, 0.015, 0.315, 0.030
            ],
            ...
            # --------------------------------------------------------
            "HEGEMON": [
                0.220, 0.065, 0.020, 0.085,
                0.055, 0.050, 0.160, 0.025,
                0.040, 0.025, 0.210, 0.045
            ],
            ...
            # --------------------------------------------------------
            "REVENGE": [
                0.165, 0.075, 0.055, 0.065,
                0.070, 0.055, 0.115, 0.040,
                0.045, 0.025, 0.250, 0.040
            ],
            ...
            # --------------------------------------------------------
            "LOYALIST": [
                0.045, 0.220, 0.055, 0.090,
                0.005, 0.170, 0.060, 0.060,
                0.060, 0.080, 0.030, 0.125
            ],
            ...
            # --------------------------------------------------------
            "FORTRESS": [
                0.020, 0.130, 0.055, 0.120,
                0.005, 0.250, 0.050, 0.105,
                0.060, 0.070, 0.015, 0.120
            ],
            ...
            # --------------------------------------------------------
            "MERCHANT": [
                0.030, 0.175, 0.035, 0.180,
                0.010, 0.095, 0.035, 0.065,
                0.250, 0.070, 0.020, 0.065
            ],
            ...
            # --------------------------------------------------------
            "AGRICULTURAL": [
                0.025, 0.120, 0.035, 0.135,
                0.005, 0.110, 0.040, 0.300,
                0.075, 0.045, 0.015, 0.095
            ],

            # --------------------------------------------------------
            # 軍國主義
            # 軍事人口、動員、全面戰爭。
            # 比 WARMONGER 更制度化，不一定亂打。
            # --------------------------------------------------------
            "MILITARY_STATE": [
                0.205, 0.060, 0.055, 0.045,
                0.055, 0.090, 0.220, 0.025,
                0.030, 0.020, 0.150, 0.045
            ],
            ...
            # --------------------------------------------------------
            "STRATEGIST": [
                0.105, 0.125, 0.100, 0.120,
                0.045, 0.105, 0.085, 0.070,
                0.085, 0.045, 0.080, 0.030
            ],
            ...
            # --------------------------------------------------------
            "CONSPIRATOR": [
                0.075, 0.135, 0.125, 0.070,
                0.190, 0.075, 0.065, 0.035,
                0.045, 0.025, 0.135, 0.025
            ],
            ...
            # --------------------------------------------------------
            "BERSERKER": [
                0.330, 0.025, 0.020, 0.025,
                0.080, 0.025, 0.180, 0.020,
                0.020, 0.010, 0.235, 0.030
            ],
            ...
            # --------------------------------------------------------
            "CONSERVATIVE": [
                0.045, 0.145, 0.050, 0.145,
                0.005, 0.145, 0.045, 0.110,
                0.100, 0.090, 0.025, 0.090
            ],
            ...
            # --------------------------------------------------------
            "INTERNATIONALIST": [
                0.035, 0.210, 0.155, 0.095,
                0.005, 0.150, 0.040, 0.050,
                0.105, 0.080, 0.015, 0.060
            ],
            ...
            # --------------------------------------------------------
            "ANTI_HEGEMON": [
                0.080, 0.110, 0.260, 0.065,
                0.025, 0.115, 0.090, 0.035,
                0.035, 0.025, 0.145, 0.010
            ],

            # --------------------------------------------------------
            # 擴張主義者
            # 和 HEGEMON 類似，但比較重視領土擴張。
            # --------------------------------------------------------
            "EXPANSIONIST": [
                0.250, 0.050, 0.025, 0.065,
                0.060, 0.045, 0.165, 0.025,
                0.030, 0.020, 0.220, 0.045
            ],
            ...
            # --------------------------------------------------------
            "SURVIVALIST": [
                0.025, 0.155, 0.080, 0.125,
                0.003, 0.190, 0.045, 0.120,
                0.075, 0.065, 0.010, 0.107
            ],
            ...
            # --------------------------------------------------------
            "DIPLOMAT": [
                0.025, 0.250, 0.080, 0.130,
                0.003, 0.130, 0.030, 0.055,
                0.160, 0.115, 0.015, 0.057
            ],

            # --------------------------------------------------------
            # 亂世軍閥
            # 利用混亂擴張勢力。
            # 比單純 WARMONGER 更重視背叛與襲擾。
            # --------------------------------------------------------
            "OPPORTUNISTIC_WARLORD": [
                0.180, 0.065, 0.045, 0.045,
                0.135, 0.045, 0.145, 0.025,
                0.025, 0.015, 0.245, 0.030
            ],
            ...
            # --------------------------------------------------------
            "BALANCER": [
                0.055, 0.175, 0.210, 0.105,
                0.015, 0.155, 0.050, 0.045,
                0.070, 0.065, 0.045, 0.065
            ],
        }

        weights = list(
            personality_weights.get(
                self.personality_type,
                personality_weights["OPPORTUNIST"]
            )
        )
        ...
        try:
            ...
            _atomic_write_json(
                "war_live_countries.json",
                {
                    ...
                    "engine_version": "亂世演算V6.6.3_滅國人口分配版",
                },
