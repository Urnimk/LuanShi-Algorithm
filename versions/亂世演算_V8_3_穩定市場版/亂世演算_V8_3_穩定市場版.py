...
WORLD_BANK_LIQUIDITY_GRANT_CAP = 60
...
# ====================================================
# 【自主學習 RL 大腦 V5】16 Actions + 11維 State + 延遲 Reward / Action Mask
# ====================================================
# V7 取消固定人格與人格探索偏好。
...
class CountryRLAgent:
    ...
    def choose_action(
        ...
        self.visit_counts[state_str] = max(0, int(self.visit_counts.get(state_str, 0) or 0)) + 1
    ...
    def learn(
        ...
        masked_next = [
            q_next[a] for a in (next_valid_actions or range(self.action_size))
            if 0 <= a < self.action_size
        ]
        target = reward + self.gamma * (max(masked_next) if masked_next else 0.0)
        q_now[action] += self.lr * (target - predict)
        key = str(int(action))
        stat = self.action_stats.setdefault(key, {"count": 0, "reward_sum": 0.0, "success": 0})
        stat["count"] = max(0, int(stat.get("count", 0) or 0)) + 1
        stat["reward_sum"] = float(stat.get("reward_sum", 0.0) or 0.0) + float(reward)
        if action_success:
            stat["success"] = max(0, int(stat.get("success", 0) or 0)) + 1
    ...
    def settle_delayed_evaluations(self, current_data, turn_counter):
        ...
        for item in self.pending_decisions:
            ...
            action = int(item.get("action", 3) or 3)
            reward = 0.0
            ...
            q = self._ensure_q_vector(str(item.get("state", "")))
            q[action] += self.lr * 0.5 * (reward - q[action])
