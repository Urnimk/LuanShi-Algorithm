...
ENGINE_VERSION = "亂世演算V8_5_9_財政決策回流版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
BALANCE = {
    ...
    "FOOD_PER_WORKER": 4.2,  # 4.2
    "WOOD_PER_WORKER": 1.6,
    "METAL_PER_WORKER": 1.7, # 1.7 
...
class CountryRLAgent:
    ...
    def choose_action(
        ...
        if strategic_action == 7 and bank is not None:
            reserved, _ = _market_order_reservations(market or {}, self.country_name)
            available = max(0, int(country_data.get("gold", 0) or 0) - reserved)
            ...
            self._last_fiscal_cost = {0: 0, 1: birth_cost, 2: production_cost,
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                if agent._last_fiscal_state is not None:
                    ...
                    next_fiscal_valid = fiscal_valid_actions(
                        c_data, get_valid_rl_actions(attacker_key, countries, alliances,
                                                     anti_hegemon_coalitions, alive_sorted),
                        next_cash, birth_cost, production_cost,
