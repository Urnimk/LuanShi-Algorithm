...
try:
    from 亂世演算_V8_4_5_戰爭勝算評估版 import (
        thread_stat_monitor,
    ...
    pattern = os.path.join(
        ...
        "亂世演算_V8_4_5_戰爭勝算評估版*.py",
    )
    candidates = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    if not candidates:
        raise
    spec = importlib.util.spec_from_file_location("亂世演算_v845_engine", candidates[0])
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V8_4_5｜戰爭勝算評估版"
