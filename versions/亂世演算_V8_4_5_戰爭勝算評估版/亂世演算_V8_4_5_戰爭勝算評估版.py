...
ENGINE_VERSION = "亂世演算V8_4_5_戰爭勝算評估版"
...
class CountryRLAgent:
    ...
    def _get_strategic_state(self, country_data, total_world_power, max_power, alive_count):
        ...
        return (
            ...
            diplomacy, old[8], phase, cash_power, contender,
        )
    ...
    def inferred_personality(self):
        ...
        for state_key, values in self.strategic_policy.q_table.items():
            ...
            weight = 1.0 + math.log1p(visits)
            for index, value in enumerate(values[:len(totals)]):
                totals[index] += float(value) * weight
                weights[index] += weight
        averages = [totals[i] / weights[i] if weights[i] else 0.0 for i in range(len(totals))]
        ...
        gap = averages[first] - averages[second]
        if gap < 0.08:
            return f"混合型：{labels[first]}／{labels[second]}"
        return labels[first]
