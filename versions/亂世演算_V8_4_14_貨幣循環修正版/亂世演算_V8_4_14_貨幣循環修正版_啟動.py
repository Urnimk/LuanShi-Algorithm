...
try:
    from 亂世演算_V8_4_14_貨幣循環修正版 import (
        thread_stat_monitor,
    ...
    pattern = os.path.join(
        ...
        "亂世演算_V8_4_14_貨幣循環修正版*.py",
    )
    candidates = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    if not candidates:
        raise
    spec = importlib.util.spec_from_file_location("亂世演算_v8414_engine", candidates[0])
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V8_4_10｜大洋發行市場版"
    ...
    def refresh_world_market_panel(self):
        ...
        self.market_bank_label.configure(
            text=(
                ...
                f"政策指數 {float(bank.get('policy_price_index', 1.0) or 1.0):.2f}｜"
                f"政策費 {int(bank.get('birth_policy_cost', 120000) or 120000):,}｜"
        ...
        self.market_stats_label.configure(
            text=(f"市場品質｜累計掛單 {placed:,}｜完整成交 {filled:,}｜到期 {expired:,}｜成交率 {fill_rate:.1f}%"
                  f"\n銀行本屆新發 {int(bank.get('market_issued_this_season', 0) or 0):,} / "
                  f"{int(bank.get('market_issue_season_budget', 0) or 0):,} {currency}（銀行金庫付款另計）")
        )
