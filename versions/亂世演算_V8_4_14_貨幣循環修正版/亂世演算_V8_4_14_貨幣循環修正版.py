...
ENGINE_VERSION = "亂世演算V8_4_14_貨幣循環修正版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
SEASON_SECONDS = SEASON_SECONDS // SEASON_YEAR // SEASON_RATIO
...
# 新手保護：新建立／亡國重建的國家，500 個模擬年內不能成為主動侵略目標。
NEWBIE_PROTECTION_TURNS = 10
...
# 價格低點只作發行觸發線；不強制所有玩家只能在這個價位以上報價。
MARKET_PRICE_FLOOR_RATE = 0.00001
MARKET_PRICE_CEILING_RATE = 2.50
WORLD_BANK_ISSUE_TRIGGER_RATE = 0.60
# 一屆新發貨幣最多佔本屆開始時流通貨幣的 5%；掛單量仍由 AI 與實際庫存決定。
WORLD_BANK_MAX_NEW_MONEY_PER_SEASON_RATE = 0.05
# 世界銀行承接價格不高於預設價的有效賣單：數量由掛單與真實庫存決定。
...
def _normalize_world_bank(bank):
    ...
    base["policy_price_index"] = max(.50, min(4.0, float(base.get("policy_price_index", 1.0) or 1.0)))
    ...
    base["policy_index_target"] = max(.50, min(4.0, float(base.get("policy_index_target", 1.0) or 1.0)))
    ...
    base["currency_rename_trigger_rate"] = float(base.get("currency_rename_trigger_rate", WORLD_BANK_ISSUE_TRIGGER_RATE))
    for key in (
...
def _world_bank_recirculate_liquidity(bank, countries, turn_counter):
    ...
    for key, data in countries.items():
        ...
        gold_pc = gold / pop
        if gold_pc < 3.0 * MONEY_RATIO:
            # 現金越短缺、人口越多，回流順位越高。
            weight = max(0.1, 3.0 * MONEY_RATIO - gold_pc) * math.sqrt(pop)
    ...
    if distributed:
        ...
        bank["last_action"] = f"逆週期流動性回流：{distributed} {bank.get('currency', WORLD_CURRENCY_NAME)}／{len(grants)} 國"
        print(f"🏦 [流動性回流] 世界銀行從既有庫存釋出 {distributed} {bank.get('currency', WORLD_CURRENCY_NAME)}，支援 {len(grants)} 個現金短缺國家。")
    return grants
...
def _market_place_ai_order(country_key, countries, market, turn_counter):
    ...
    if shortage_ratio < 0.95 and available_gold >= 25 * MONEY_RATIO:
        side = "BUY"
        ...
        side = "SELL"
        resource = surplus_res
    elif available_gold >= 180 * MONEY_RATIO:
        side = "BUY"
        resource = shortage_res
    else:
        return None
    ...
    base_price = _clamp(
        ...
        _market_price_floor(resource),
        _market_price_ceiling(resource),
    )
    ...
    price = round(_clamp(price, _market_price_floor(resource), _market_price_ceiling(resource)), 2)
    qty = max(0, int(qty))
    if qty <= 0:
        return None
...
def _market_layer_state(data, market):
    ...
    liquidity = "LOW" if gold_pc < 1.0 * MONEY_RATIO else "OK" if gold_pc < 4.0 * MONEY_RATIO else "HIGH"
    trends = []
    spreads = []
    for resource in MARKET_RESOURCES:
...
def _market_place_layered_order(country_key, countries, market, turn_counter, market_action, price_action, size_action):
    ...
    price = round(_clamp(price, max(_market_price_floor(resource), reference * 0.80), min(_market_price_ceiling(resource), reference * 1.20)), 2)
    pop = max(1, int(data.get("pop_total", 1) or 1))
    ...
    liquidity_factor = _clamp(max(0.0, float(data.get("gold", 0) or 0)) / max(50.0 * MONEY_RATIO, pop * 2.0 * MONEY_RATIO), 0.35, 1.5)
...
def _market_price_floor(resource):
    """回傳該資源的市場價格下限；避免長期單邊賣壓把價格打到接近 0。"""
    seed = float(MARKET_RESOURCE_META[resource]["seed_price"])
    return max(0.05, seed * MARKET_PRICE_FLOOR_RATE)


def _market_price_ceiling(resource):
    """回傳該資源的市場價格上限，避免短期搶購無限拉高。"""
    seed = float(MARKET_RESOURCE_META[resource]["seed_price"])
    return max(_market_price_floor(resource), seed * MARKET_PRICE_CEILING_RATE)


def _world_bank_issue_for_market(bank, countries, market, resource, turn_counter):
    """跌至預設低點後，以新發行貨幣按種子價承接真實賣單。

    只有實際交割才增加發行量、賣方餘額與價格紀錄；每商品每回合限一次。
    """
    stat = market["stats"][resource]
    seed = float(MARKET_RESOURCE_META[resource]["seed_price"])
    if float(stat.get("last_price", seed) or seed) > seed * WORLD_BANK_ISSUE_TRIGGER_RATE:
        return []
    turns = bank.setdefault("market_issue_last_turn", {})
    if int(turns.get(resource, -1)) == int(turn_counter):
        return []
    turns[resource] = int(turn_counter)
    # 每屆新發總額有上限，現有銀行金庫可先用於購買；絕不限定 AI 掛單數量。
    season = int(bank.get("current_season_round", 1) or 1)
    if int(bank.get("market_issue_season", 0) or 0) != season:
        bank["market_issue_season"] = season
        bank["market_issued_this_season"] = 0
        circulation = sum(max(0, int(c.get("gold", 0) or 0)) for c in countries.values())
        bank["market_issue_season_budget"] = max(0, int(circulation * WORLD_BANK_MAX_NEW_MONEY_PER_SEASON_RATE))
    available_money = (max(0, int(bank.get("treasury", 0) or 0))
                       + max(0, int(bank["market_issue_season_budget"]) - int(bank["market_issued_this_season"])))
    purchases = []
    sells = sorted(
        (o for o in market.get("orders", [])
         if o.get("resource") == resource and o.get("side") == "SELL"
         and int(o.get("qty", 0) or 0) > 0 and float(o.get("price", 0) or 0) <= seed),
        key=lambda o: (float(o.get("price", 0) or 0), int(o.get("turn", 0) or 0), str(o.get("id", ""))),
    )
    for order in sells:
        seller = countries.get(order.get("country"))
        if not seller or not seller.get("is_alive", True):
            continue
        _, reserved = _market_order_reservations(market, order.get("country"), order.get("id"))
        available = max(0, int(seller.get(resource, 0) or 0) - reserved.get(resource, 0))
        qty = min(int(order.get("qty", 0) or 0), available, int(available_money // seed))
        if qty <= 0:
            continue
        gross = int(round(qty * seed))
        if gross <= 0:
            continue
        from_treasury = min(int(bank.get("treasury", 0) or 0), gross)
        newly_issued = gross - from_treasury
        bank["treasury"] = int(bank.get("treasury", 0) or 0) - from_treasury
        bank["issued_total"] = int(bank.get("issued_total", 0) or 0) + newly_issued
        bank["market_issued_total"] = int(bank.get("market_issued_total", 0) or 0) + newly_issued
        bank["market_issued_this_season"] += newly_issued
        available_money -= gross
        seller["gold"] = int(seller.get("gold", 0) or 0) + gross
        seller[resource] = int(seller.get(resource, 0) or 0) - qty
        bank.setdefault("market_reserves", {})[resource] = int(bank.get("market_reserves", {}).get(resource, 0) or 0) + qty
        order["qty"] = int(order.get("qty", 0) or 0) - qty
        if order["qty"] == 0:
            market["orders_filled"] = int(market.get("orders_filled", 0) or 0) + 1
        trade = {
            "turn": int(turn_counter), "resource": resource,
            "resource_name": MARKET_RESOURCE_META[resource]["name"],
            "qty": qty, "price": seed, "gross_gold": gross, "fee_gold": 0,
            "buyer": WORLD_BANK_NAME, "buyer_name": WORLD_BANK_NAME,
            "seller": order.get("country"),
            "seller_name": seller.get("display_name", order.get("country")),
            "buy_order_id": None, "sell_order_id": order.get("id"),
            "currency_issued": newly_issued, "bank_treasury_used": from_treasury,
        }
    ...
    if purchases:
        quantity = sum(t["qty"] for t in purchases)
        total = sum(t["gross_gold"] for t in purchases)
        minted = sum(t["currency_issued"] for t in purchases)
        bank["market_issue_operations"] = int(bank.get("market_issue_operations", 0) or 0) + 1
        currency = bank.get("currency", WORLD_CURRENCY_NAME)
        bank["last_action"] = f"市場購買：支付 {total} {currency}（新發 {minted}），按預設價購買 {quantity} {MARKET_RESOURCE_META[resource]['name']}"
        stat["last_price"] = seed
        stat["traded_volume"] = int(stat.get("traded_volume", 0) or 0) + quantity
        stat["traded_value"] = int(stat.get("traded_value", 0) or 0) + total
        stat["last_trade_volume"] = quantity
        stat["last_trade_turn"] = int(turn_counter)
        stat["trade_count"] = int(stat.get("trade_count", 0) or 0) + len(purchases)
        stat["price_history"] = (list(stat.get("price_history", []) or []) + [seed])[-MARKET_PRICE_HISTORY_MAX:]
        recent = [t for t in market["recent_trades"] if t.get("resource") == resource][-MARKET_VWAP_WINDOW:]
        volume = sum(int(t["qty"]) for t in recent)
        if volume:
            stat["vwap_10"] = round(sum(float(t["price"]) * int(t["qty"]) for t in recent) / volume, 4)
        print(f"🏦 [市場發行] {MARKET_RESOURCE_META[resource]['name']}觸及低點；世界銀行支付 {total} {currency}（其中新發 {minted}），以每單位 {seed:.2f} 購買 {quantity} 單位。")
    return purchases


def _world_bank_release_reserves(bank, countries, market, turn_counter):
    """實物回到國家，買方付款進銀行金庫；沒有新發貨幣。"""
    trades = []
    reserves = bank.setdefault("market_reserves", {})
    for resource in MARKET_RESOURCES:
        seed = float(MARKET_RESOURCE_META[resource]["seed_price"])
        buyers = sorted((o for o in market.get("orders", [])
                         if o.get("resource") == resource and o.get("side") == "BUY"
                         and int(o.get("qty", 0) or 0) > 0
                         and float(o.get("price", 0) or 0) >= seed),
                        key=lambda o: (-float(o.get("price", 0) or 0),
                                       int(o.get("turn", 0) or 0), str(o.get("id", ""))))
        for order in buyers:
            reserve = max(0, int(reserves.get(resource, 0) or 0))
            buyer = countries.get(order.get("country"))
            if reserve <= 0 or not buyer or not buyer.get("is_alive", True):
                break
            other_gold, _ = _market_order_reservations(market, order.get("country"), order.get("id"))
            available = max(0, int(buyer.get("gold", 0) or 0) - other_gold)
            qty = min(reserve, int(order.get("qty", 0) or 0), int(available // seed))
            if qty <= 0:
                continue
            gross = int(round(qty * seed))
            buyer["gold"] = int(buyer.get("gold", 0) or 0) - gross
            buyer[resource] = int(buyer.get(resource, 0) or 0) + qty
            bank["treasury"] = int(bank.get("treasury", 0) or 0) + gross
            reserves[resource] = reserve - qty
            order["qty"] = int(order.get("qty", 0) or 0) - qty
            if order["qty"] == 0:
                market["orders_filled"] = int(market.get("orders_filled", 0) or 0) + 1
            trade = {
                ...
                "resource_name": MARKET_RESOURCE_META[resource]["name"],
                "qty": qty, "price": seed, "gross_gold": gross, "fee_gold": 0,
                "buyer": order.get("country"),
                "buyer_name": buyer.get("display_name", order.get("country")),
                "seller": WORLD_BANK_NAME, "seller_name": WORLD_BANK_NAME,
                "buy_order_id": order.get("id"), "sell_order_id": None,
                "currency_issued": 0,
            }
            trades.append(trade)
            market.setdefault("recent_trades", []).append(trade)
            ...
            stat["traded_volume"] = int(stat.get("traded_volume", 0) or 0) + qty
            stat["traded_value"] = int(stat.get("traded_value", 0) or 0) + gross
            stat["trade_count"] = int(stat.get("trade_count", 0) or 0) + 1
            stat["price_history"] = (list(stat.get("price_history", []) or []) + [seed])[-MARKET_PRICE_HISTORY_MAX:]
            recent = [t for t in market["recent_trades"] if t.get("resource") == resource][-MARKET_VWAP_WINDOW:]
            volume = sum(int(t.get("qty", 0) or 0) for t in recent)
            if volume:
                stat["vwap_10"] = round(sum(float(t["price"]) * int(t["qty"]) for t in recent) / volume, 4)
    return trades
...
def _market_match_orders(countries, market, bank, turn_counter):
    ...
    # 幣名只在屆末檢視本屆多種商品的實際國家間成交，不依單筆價格輪換。
    # 銀行金庫出售舊有實物，所得回到金庫；先回流實物再視低價狀況購買。
    trades.extend(_world_bank_release_reserves(bank, countries, market, turn_counter))
    # 低價觸線時以實際交割發行貨幣，價格紀錄只反映真實成交。
    for resource in MARKET_RESOURCES:
        trades.extend(_world_bank_issue_for_market(bank, countries, market, resource, turn_counter))
    _market_prune_orders(market, countries, turn_counter)
    return trades
...
def _alliance_resource_aid(donor_key, countries, alliances):
    ...
    gold_surplus = max(0, int(donor.get("gold", 0) or 0) - 180 * MONEY_RATIO)
    gold_amount = min(80 * MONEY_RATIO, int(gold_surplus * 0.15))
    if gold_amount > 0:
...
def _alliance_layer_valid_actions(country_key, countries, alliances):
    ...
    if not members:
        return [0]
    valid = [0, 1]
    if any(data.get(res, 0) > _market_target_stock(data, res) * 1.10 for res in MARKET_RESOURCES) or data.get("gold", 0) > 180 * MONEY_RATIO:
        valid.extend((2, 3, 4))
...
def _award_season_champion_rewards(champion_key, countries, alliances, world_bank, turn_counter):
    ...
    champion_paid = _world_bank_grant(
        world_bank, champion, SEASON_CHAMPION_GOLD_REWARD, reason="賽季冠軍獎金"
    )
    ...
    pool = SEASON_CHAMPION_ALLIANCE_POOL if allies else 0
...
COALITION_FORMATION_BOUNTY = 10 * MONEY_RATIO
COALITION_VICTORY_BOUNTY = 60 * MONEY_RATIO
...
def select_international_dread_coalition_target(countries, alive_sorted):
    """忌憚值達標即建立持續圍剿窗口，不再讓多重抽籤把觸發率壓到近乎為零。"""
    candidates = [
    ...
    if not candidates:
        return None
    return max(candidates, key=lambda k: int(countries[k].get("international_dread", 0) or 0))
...
def _apply_infrastructure_investment(data, bank, gain=None):
    ...
    if (
        ...
        or int(data.get("gold", 0) or 0) < gold_cost
    ...
    return {
        "gain": actual_gain, "gold": gold_cost,
        "wood": wood_cost, "metal": metal_cost,
    }
...
def get_valid_rl_actions(attacker_key, countries, alliances, anti_hegemon_coalitions, alive_sorted):
    ...
    if (
        infrastructure < INFRASTRUCTURE_MAX
        and gold >= infrastructure_gold_cost
    ...
    if gold >= 25 * MONEY_RATIO or max(food, wood * 2, metal * 2) >= 150:
        valid.add(8)

    # 9 聲望修復：改以世界共同貨幣支付外交成本。
    if c.get("infamy", 0) > 0 and gold >= policy_cost(60 * MONEY_RATIO, c):
        valid.add(9)
    ...
    # 12 鼓勵生育：需要政策預算。
    if gold >= policy_cost(BIRTH_POLICY_GOLD_COST, c):
        valid.add(12)

    # 13 增產補貼：需要政策預算。
    if gold >= policy_cost(PRODUCTION_POLICY_GOLD_COST, c):
        valid.add(13)
    ...
    if current_alliance in alliances:
        ...
        if partners and (gold > 180 * MONEY_RATIO or food > 250 or wood > 150 or metal > 100):
            valid.add(15)
...
class CountryRLAgent:
    ...
    def _get_state_key(self, country_data, total_world_power, max_power, alive_count):
        ...
        wealth_pc = (food + wood * 1.8 + metal * 3.2 + gold / MONEY_RATIO) / pop
        if wealth_pc < 5:
        ...
        gold_pc = gold / pop
        if gold_pc < 1.0 * MONEY_RATIO:
            liquidity = "CASH_CRISIS"
        elif gold_pc < 3.0 * MONEY_RATIO:
            liquidity = "CASH_LOW"
        elif gold_pc < 8.0 * MONEY_RATIO:
            liquidity = "CASH_OK"
        else:
            liquidity = "CASH_RICH"
    ...
    def _get_strategic_state(self, country_data, total_world_power, max_power, alive_count):
        ...
        liquidity_turns = (
            max(0.0, float(country_data.get("gold", 0) or 0)) / MONEY_RATIO
        ) / obligations
    ...
    def schedule_delayed_evaluation(self, state_data, action, turn_counter, action_success):
        if not action_success or action not in (0, 1, 12, 13, 14, 15):
            return
        horizon = {0: 8, 1: 10, 12: BIRTH_POLICY_DURATION, 13: PRODUCTION_POLICY_DURATION, 14: 5, 15: 5}[action]
...
def resolve_persistent_war_round(
    ...
    # 小國／殘國不能因「士兵已清零」卻永遠拖著不滅。
    # 當防守方已無士兵且人口極少，視為政權失去維持能力，直接進入滅國結算。
    ...
    if def_total_soldiers <= 0 and def_total_pop <= 12 and round_no >= 2:
        ...
        if _war_execute_capital_fall(loser_key, att_members, countries, note_population_loss, turn_counter, contribution_weights=war.get("attacker_troop_contribution", {}), attacker_name=war.get("attacker_name", "")):
            return {"ended": True, "winner": "ATTACKER", "reason": "殘國已無軍隊，直接滅國"}
        ...
        if def_ratio <= 0.40 and avg_fort < 30:
            ...
            if _war_execute_capital_fall(
                ...
                print(
                    ...
                    f"立即進入滅國結算。{decision_reason}。"
                )
                return {"ended": True, "winner": "ATTACKER", "reason": "求和遭拒，直接滅國"}
...
def thread_stat_monitor():
    ...
    if not is_loaded:
            ...
            for name in base_country_names:
                ...
                countries[name] = {  # 設定國家初始屬性
                    ...
                    "newbie_protection_until_turn": NEWBIE_PROTECTION_TURNS,
    ...
    if old_index_schema < 2 and loaded_season_elapsed > 0:
        print(f"📊 [價格指數修正] 舊版庫存分母造成的低估已重設；當屆政策指數 {world_bank['policy_price_index']:.3f}。")
    # 主遊戲運行迴圈 (Game Loop)
    while True:
        _wait_for_simulation_permission()
        world_bank["current_season_round"] = int(count_round)
        season_progress = _clamp((time.time() - start_time) / max(1.0, float(time_total)), 0.0, 1.0)
                    ...
                    if war.get("is_coalition_war"):
                        ...
                        for member in att_members:
                            ...
                            paid = _world_bank_pay_bounty(
                                world_bank, countries[member], COALITION_VICTORY_BOUNTY,
                                reason="反霸權圍剿勝利懸賞",
                            )
                            bounty_total += paid
                ...
                if action == 0:
                    ...
                    if possible_defenders:
                        ...
                        viable_defenders = [
                            ...
                            if matchup_cache[k]["win_chance"] >= 0.35
                            or (
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                        ...
                        if (
                            ...
                            if initiator_willingness >= 0.18:
                                ...
                                if len(accepted) >= COALITION_MIN_MEMBERS:
                                    ...
                                    for m in accepted:
                                        paid = _world_bank_pay_bounty(
                                            world_bank, countries[m], COALITION_FORMATION_BOUNTY,
                                            reason="反霸權聯軍成立懸賞",
                                        )
                ...
                # Action 3: 基礎建設投資（支付現有資產，換取永久效率）
                    ...
                    if investment:
                        ...
                        action_value = gain
                        print(
                            f"[基礎建設] 【{c_data['display_name']}】支付大洋 {investment['gold']}、"
                            f"木材 {investment['wood']}、金屬 {investment['metal']}，基建 +{gain}，"
                ...
                # ----------------------------------------------------
                    ...
                    if old_infamy > 0 and gold_stock >= policy_cost(60 * MONEY_RATIO, world_bank):
                        ...
                        reduction = random.randint(14, 28) + int(min(10, diplomacy_cost / max(1, policy_cost(20 * MONEY_RATIO, world_bank))))
                        c_data["infamy"] = max(0, old_infamy - reduction)
                        action_success = True
                        action_value += old_infamy - c_data["infamy"]
                        print(
                            f"🤝 [外交修復] 【{c_data['display_name']}】支付 {diplomacy_cost} {world_bank.get('currency', WORLD_CURRENCY_NAME)}進行外交斡旋，"
                ...
                # ----------------------------------------------------
                    ...
                    if action_success:
                        ...
                        print(
                            f"👶 [鼓勵生育] 【{c_data['display_name']}】投入 {spent} {world_bank.get('currency', WORLD_CURRENCY_NAME)}，"
                ...
                # ----------------------------------------------------
                    ...
                    if action_success:
                        ...
                        print(
                            f"🏭 [增產補貼] 【{c_data['display_name']}】投入 {spent} {world_bank.get('currency', WORLD_CURRENCY_NAME)}，"
                ...
                # ----------------------------------------------------
                    ...
                    if aid:
                        ...
                        print(
                            ...
                            f"【{countries[target].get('display_name', target)}】提供 {amount} 大洋，"
                            f"以改善雙邊關係。"
                        )
            ...
            if not data["is_alive"]:
                if dead_mode == 0:  # 冷卻模式
                    if data["death_respawn_cooldown"] > 0:
                        ...
                        # 復國後視為新手國家：500 年內不會被 AI 當成主動侵略目標。
                        data["newbie_protection_until_turn"] = int(turn_counter) + NEWBIE_PROTECTION_TURNS
        ...
        rank_text = f"===== 亂世演算 第 {count_round} 屆 戰力大戰 ===== 剩餘 {int(time_total - (time.time() - start_time))} 秒結束\n"
        rank_text += (
        ...
        if (time.time() - start_time) > time_total:
            ...
            if settle_currency_name(world_bank, world_market, MARKET_RESOURCE_META,
                                    WORLD_CURRENCY_NAME_OPTIONS, count_round, turn_counter):
                print(f"💱 [貨幣名稱輪替] 連續兩屆多種商品低價成交，貨幣名稱改為 {world_bank['currency']}；"
                      "所有國家餘額、銀行金庫、商品價格和發行總額均維持 1:1 不變。")
