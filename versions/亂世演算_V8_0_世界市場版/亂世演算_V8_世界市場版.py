...
WORLD_BANK_RESPAWN_GRANT = 120
WORLD_BANK_TRANSACTION_FEE_RATE = 0.01

MARKET_RESOURCES = ("food", "wood", "metal")
MARKET_RESOURCE_META = {
    "food": {"name": "糧食", "seed_price": 1.00},
    "wood": {"name": "木材", "seed_price": 1.80},
    "metal": {"name": "金屬", "seed_price": 3.20},
}
MARKET_ORDER_TTL = 4
...
def _market_place_ai_order(country_key, countries, market, turn_counter):
    ...
    if shortage_ratio < 0.95 and gold >= 25:
        side = "BUY"
        resource = shortage_res
    elif surplus_ratio > 1.30:
        side = "SELL"
        resource = surplus_res
    elif gold < 90 and surplus_ratio > 1.05:
        side = "SELL"
        resource = surplus_res
    elif gold >= 180:
        side = "BUY"
    ...
    if side == "BUY":
        ...
        affordable = int(gold / max(0.01, price))
    ...
    # 同一國同一資源只保留最新一張掛單，避免洗版與重複鎖單。
    market["orders"] = [
        o for o in market.get("orders", [])
        if not (o.get("country") == country_key and o.get("resource") == resource)
    ]
...
def _market_match_orders(countries, market, bank, turn_counter):
    ...
    for resource in MARKET_RESOURCES:
        while True:
            ...
            unit_price = round((float(buy["price"]) + float(sell["price"])) / 2.0, 2)
            buyer_gold = max(0, int(buyer.get("gold", 0) or 0))
            seller_stock = max(0, int(seller.get(resource, 0) or 0))
            affordable_qty = int(buyer_gold / max(0.01, unit_price))
            qty = min(int(buy.get("qty", 0) or 0), int(sell.get("qty", 0) or 0), seller_stock, affordable_qty)
            if qty <= 0:
                if seller_stock <= 0:
                    sell["qty"] = 0
                if affordable_qty <= 0:
            ...
            fee = int(round(gross * WORLD_BANK_TRANSACTION_FEE_RATE))
            if gross >= 50:
                fee = max(1, fee)
            fee = min(fee, gross)
            net = gross - fee
...
# ====================================================
# 【自主學習 RL 大腦 V4】16 Actions + 11維 State + 世界市場/金幣 Reward
# ====================================================
# V7 取消固定人格與人格探索偏好。
# V8 因新增金幣狀態與 4 個新行動，RL_VERSION 升為 4；舊 Q-table 會乾淨重學。
RL_VERSION = 4
    ...
    def calculate_reward(
        ...
        def resource_score(d):
            return (
                ...
                + max(0.0, float(d.get("wood", 0) or 0)) * 1.8
                + max(0.0, float(d.get("metal", 0) or 0)) * 3.2
...
def thread_stat_monitor():
    ...
    while True:
        ...
        # V8：本輪所有主動掛牌完成後，再做一次全市場撮合與過期單清理。
        _market_match_orders(countries, world_market, world_bank, turn_counter)
