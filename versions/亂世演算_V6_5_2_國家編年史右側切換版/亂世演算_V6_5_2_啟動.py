...
from 亂世演算_V6_5_2_國家編年史右側切換版 import thread_stat_monitor
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V6.5.2｜國家編年史右側切換・戰力權重投票・世代分流"
...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        # ----------------------------------------------------
        # ➡️ 右側區域：單一內容區
        #    預設顯示「即時戰局」；按鈕切換到國家編年史。
        #    不再以 Notebook 分頁顯示「國家編年史」標籤。
        # ----------------------------------------------------
        self.right_host = tk.Frame(
            self.root,
            bg=COLOR_BG_MAIN,
        )
        self.right_host.grid(
            row=0,
            column=1,
            sticky="nsew",
            padx=(0, 10),
        ...
        self.right_host.grid_rowconfigure(0, weight=1)
        self.right_host.grid_columnconfigure(0, weight=1)

        # ----------------------------------------------------
        # 右側頁面 A：即時戰局
        # ----------------------------------------------------
        self.right_live_page = tk.Frame(
            self.right_host,
            bg=COLOR_BG_MAIN,
        )
        self.right_live_page.grid(
            row=0,
            column=0,
            sticky="nsew",
        )
        self.right_live_page.grid_rowconfigure(1, weight=1)
        self.right_live_page.grid_columnconfigure(0, weight=1)

        self.right_live_header = tk.Frame(
            self.right_live_page,
            bg=COLOR_BG_MAIN,
        )
        self.right_live_header.grid(
            row=0,
            column=0,
            sticky="ew",
            pady=(0, 5),
        )
        self.right_live_header.grid_columnconfigure(0, weight=1)

        tk.Label(
            self.right_live_header,
            text=" 🌍 即時戰局 ",
            bg=COLOR_BG_MAIN,
            fg=COLOR_FG_WHITE,
            font=FONT_UI_BOLD,
        ...
        # 這顆按鈕就是原本「📜 國家編年史」Tab 的替代品。
        self.right_chronicle_button = ttk.Button(
            self.right_live_header,
            text="📜 國家編年史",
            command=self.open_selected_country_chronicle,
        )
        self.right_chronicle_button.grid(
            row=0,
            column=1,
            sticky="e",
        )
        ...
        # 右側頁面 B：國家編年史
        # ----------------------------------------------------
        self.right_chronicle_page = tk.Frame(
            self.right_host,
            bg=COLOR_BG_MAIN,
        )
        self.right_chronicle_page.grid(
            row=0,
            column=0,
            sticky="nsew",
        )
        ...
        # 使用者要求：編年史畫面要有「返回詳細資訊」按鈕。
        self.chronicle_back_button = ttk.Button(
            self.chronicle_top,
            text="← 返回詳細資訊",
            command=self.return_from_chronicle,
        )
        ...
        chronicle_specs = (
            ...
            ("end_reason", "結束原因", 0, "w"),
            ("peak_power", "最高戰力（年份）", 200, "e"),
            # ("peak_population", "最高人口", 105, "e"),
            # ("best_rank", "最佳排名", 80, "center"),
        )
        ...
        for col, title, width, anchor in chronicle_specs:
            ...
            self.chronicle_tree.column(
                ...
                minwidth=60,
                anchor=anchor,
                stretch=(col in {"name", "end_reason"}),
            )
        ...
        self.chronicle_table_frame.grid_rowconfigure(
            0,
            weight=1,
        )
        self.chronicle_table_frame.grid_columnconfigure(
            0,
            weight=1,
        )
    ...
    def open_selected_country_chronicle(self):
        """
        將右半側切換成目前國家的編年史。
        不再依靠 Notebook 的「國家編年史」Tab。
        """
        selected = self.country_cb.get().strip()
        country_key = self.resolve_country_key(selected)

        # 若左側尚未選國，改採編年史下拉或第一個國家。
        if (
            not country_key
            or country_key not in self.raw_countries_data
        ):
            fallback = self.chronicle_country_cb.get().strip()
            country_key = self.resolve_country_key(fallback)

        if (
            not country_key
            or country_key not in self.raw_countries_data
        ):
            if self.raw_countries_data:
                country_key = next(iter(self.raw_countries_data))
            else:
                return
        ...
        # 只切換右半側內容；左側詳細資訊保持原樣。
        self.right_chronicle_page.tkraise()
        self.chronicle_tree.focus_set()


    def return_from_chronicle(self):
        """
        編年史的「返回詳細資訊」：
        右半側回到即時戰局，同時左側保持/切回該國詳細資訊。
        """
        selected = self.chronicle_country_cb.get().strip()
        country_key = self.resolve_country_key(selected)
    ...
    def on_country_selected(self, event=None):
        ...
        basic_rows = [
            ...
            (
                "AI / 惡名 / 探索",
                f"{c.get('ai_personality', '尚未同步')}｜{c.get('infamy', 0)}｜"
            ...
            if os.path.exists("war_live_countries.json"):
                ...
                for key, data in self.raw_countries_data.items():
                    ...
                    if not data.get("is_alive", True):
                        ...
                        status = "存活"
                    country_rows.append(
                        (disp_name, gen, pwr, pop, soldiers, food, status)
                    )
