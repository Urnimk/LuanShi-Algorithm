...
ENGINE_VERSION = "亂世演算V8_5_2_年度稅與當前經驗版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
def _default_world_bank():
    return {
        ...
        "annual_tax_last_year": 0,
        "annual_tax_collected": 0,
        "annual_tax_redistributed": 0,
...
def _normalize_world_bank(bank):
    ...
    for key in (
        ...
        "annual_tax_collected", "annual_tax_redistributed",
    ):
        base[key] = max(0, int(base.get(key, 0) or 0))
    return base
...
def _world_bank_recirculate_liquidity(bank, countries, turn_counter):
    ...
    treasury = max(0, int(bank.get("treasury", 0) or 0))
    distributable = min(
        max(0, treasury - policy_cost(WORLD_BANK_MIN_RESERVE, bank)),
    ...
    for key, data in countries.items():
        ...
        gold_pc = gold / pop
        if gold_pc < policy_cost(3 * MONEY_RATIO, bank):
            # 現金越短缺、人口越多，回流順位越高。
            weight = max(0.1, policy_cost(3 * MONEY_RATIO, bank) - gold_pc) * math.sqrt(pop)
    ...
    for index, (key, weight, gold_pc) in enumerate(sorted(candidates, key=lambda x: (x[2], -x[1]))):
        ...
        amount = min(policy_cost(WORLD_BANK_LIQUIDITY_GRANT_CAP, bank), remaining, max(1, proportional))
        data = countries[key]
...
def _diplomatic_gold_aid(donor_key, countries, alliances):
    ...
    if donor_gold < policy_cost(DIPLOMACY_GOLD_MIN, donor):
        return None
...
def _alliance_layer_valid_actions(country_key, countries, alliances):
    ...
    if not members:
        return [0]
    valid = [0, 1]
    if any(data.get(res, 0) > _market_target_stock(data, res) * 1.10 for res in MARKET_RESOURCES) or data.get("gold", 0) > policy_cost(180 * MONEY_RATIO, data):
        valid.extend((2, 3, 4))
...
def get_valid_rl_actions(attacker_key, countries, alliances, anti_hegemon_coalitions, alive_sorted):
    ...
    if gold >= policy_cost(DIPLOMACY_GOLD_MIN, c) and len(alive_sorted) >= 2:
        valid.add(14)
    ...
    if current_alliance in alliances:
        ...
        if partners and (gold > policy_cost(180 * MONEY_RATIO, c) or food > 250 or wood > 150 or metal > 100):
            valid.add(15)
...
class CountryRLAgent:
    ...
    def _get_state_key(self, country_data, total_world_power, max_power, alive_count):
        ...
        wealth_pc = (food + wood * 1.8 + metal * 3.2 + gold / max(1, policy_cost(MONEY_RATIO, country_data))) / pop
        if wealth_pc < 5:
        ...
        gold_pc = gold / pop
        if gold_pc < policy_cost(1 * MONEY_RATIO, country_data):
...
def thread_stat_monitor():
    ...
    while True:
        ...
        annual_tax = collect_and_redistribute(world_bank, countries, turn_counter)
        if annual_tax['collected']:
            print(f"🏦 [年度累進稅] 共收 {annual_tax['collected']:,} "
                  f"{world_bank.get('currency', WORLD_CURRENCY_NAME)}，同年度全額回流缺錢國家；"
                  "最高稅率 40%，最低 10%。")
        ...
        if (time.time() - start_time) > time_total:
            ...
            changed_resource = settle_currency_name(world_bank, world_market, MARKET_RESOURCE_META,
