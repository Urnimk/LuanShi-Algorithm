...
from 世紀帝國_AI策略強化版_戰國模式V5_聯盟議會版 import thread_stat_monitor
...
# ------------------------------------------------------------------------------
# 主視窗頂部標題列所顯示的文字名稱
WINDOW_TITLE = "亂世演算 V5｜聯盟議會・世界局勢・事件・政變"
...
class GameInterfaceUI:
    ...
    def on_alliance_selected(self, event=None):
        ...
        if is_coalition:
            ...
            last_vote = a.get("last_war_vote", {}) or {}
            if isinstance(last_vote, dict) and last_vote:
                vote_kind = str(last_vote.get("kind", "戰爭表決"))
                vote_target = str(last_vote.get("target_name", "未知目標"))
                vote_result = str(last_vote.get("result", ""))

                if vote_kind == "進攻表決":
                    vote_text = (
                        f"{vote_result}｜{vote_target}｜"
    ...
    def on_country_selected(self, event=None):
        ...
        basic_rows = [
            ...
            (
                "世代 / 政權",
                f"第 {c.get('gen', 1)} 代｜{c.get('regime_type', '正統政權')}",
            ),
            (
                "戰略 / 世界",
                (
