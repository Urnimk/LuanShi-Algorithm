...
from 世紀帝國_AI策略強化版_戰國模式V1_雙線作戰版 import thread_stat_monitor
...
class GameInterfaceUI:
    ...
    def get_country_war_details(self, country_key):
        ...
        for war_id, war in (self.raw_active_wars or {}).items():
            ...
            # 若同一國同時參加兩場戰爭，依 started_turn / war_id 排序，
            # 第二場在「立場」欄直接標示「第二線」。
            active_war_ids = []
            for _wid, _war in (self.raw_active_wars or {}).items():
                ...
                if country_key in _participants or display_name in _participants:
                    active_war_ids.append(
                        (
                            int(_war.get("started_turn", 0) or 0),
                            str(_wid),
                        )
                    )

            active_war_ids.sort(key=lambda x: (x[0], x[1]))
            ordered_ids = [_wid for _turn, _wid in active_war_ids]
