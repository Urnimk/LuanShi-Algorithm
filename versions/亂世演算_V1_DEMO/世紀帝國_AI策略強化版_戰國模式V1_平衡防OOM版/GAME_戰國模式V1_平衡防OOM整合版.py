...
from 世紀帝國_AI策略強化版_戰國模式V1_平衡防OOM版 import thread_stat_monitor
...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        self.country_record_tree.column("item", width=125, anchor="w", stretch=False)
        self.country_record_tree.column("value", width=455, anchor="w", stretch=True)
        self.country_record_tree.pack(fill="both", expand=True, padx=4, pady=4)
    ...
    def on_country_selected(self, event=None):
        ...
        records = (
            "、".join(map(str, c.get("championship_records", [])[-3:]))
            if c.get("championship_records")
            else "無"
        )
        ...
        record_rows = [
            ...
            (
                "奪冠紀錄",
                f"{c.get('championship_count', 0)} 次｜近3次：{self._compact_text(records, 38)}",
            ),
            (
                "近期攻擊來源",
                self._compact_text(attackers, 48),
            ),
        ]
