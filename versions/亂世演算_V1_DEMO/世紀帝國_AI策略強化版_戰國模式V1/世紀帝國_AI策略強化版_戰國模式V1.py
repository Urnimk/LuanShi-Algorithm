...
def _war_country_in_active_war(country_key, active_wars):
    for war in active_wars.values():
        if country_key in (war.get("attacker_members") or []):
            return True
        if country_key in (war.get("defender_members") or []):
            return True
    return False
...
def resolve_persistent_war_round(war_id, war, countries, turn_counter, note_population_loss):
    ...
    att_s = _war_total_soldiers(att_members, countries)
    def_s = _war_total_soldiers(def_members, countries)
    ...
    cur_att = _war_total_soldiers(att_members, countries)
    cur_def = _war_total_soldiers(def_members, countries)
    ...
    while True:
        ...
        if len(alive_sorted) >= 2:
            ...
            for attacker_key in active_countries:
                ...
                # 已在正式戰爭中的國家仍可發展、加固、動員、貿易、休養，
                # 但不另開第二戰線、不背叛、不另組聯軍、不做邊境襲擾。
                if _war_country_in_active_war(attacker_key, active_wars):
                    valid_actions = [
                ...
                if action == 0:
                    ...
                    if possible_defenders:
                        ...
                        for existing_id, existing_war in active_wars.items():
                            participants = set(existing_war.get("attacker_members", [])) | set(
                                existing_war.get("defender_members", [])
                            )
                            if attacker_key in participants or defender_key in participants:
                                existing_conflict = existing_id
                                break
