...
class GameInterfaceUI:
    def __init__(self, root):
        ...
        self.style.configure("Treeview", background=COLOR_BG_FIELD, foreground=COLOR_FG_WHITE, fieldbackground=COLOR_BG_FIELD, rowheight=25)
        ...
        tk.Label(self.select_frame, text="選擇國家：", bg=COLOR_BG_MAIN, fg=COLOR_FG_WHITE, font=FONT_UI_BOLD).pack(side="left")
        self.country_cb = ttk.Combobox(self.select_frame, font=FONT_UI_NORMAL)
        ...
        self.detail_text.pack(fill="x", padx=10, pady=(0, 5))

        self.country_log_group = tk.LabelFrame(
            self.tab_detail,
            text=f" 📜 近 {MAX_SPECIFIC_LOG_LINES} 次國家／關聯聯盟事件 LOG ",
            bg=COLOR_BG_MAIN,
            fg=COLOR_FG_TEXT,
            font=FONT_UI_BOLD
        )
        self.country_log_group.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        ...
        self.country_log_text.pack(fill="both", expand=True, padx=5, pady=5)
        ...
        self.ally_select_frame.pack(fill="x", padx=10, pady=5)

        tk.Label(self.ally_select_frame, text="選擇聯盟：", bg=COLOR_BG_MAIN, fg=COLOR_FG_WHITE, font=FONT_UI_BOLD).pack(side="left")
        self.ally_cb = ttk.Combobox(self.ally_select_frame, font=FONT_UI_NORMAL)
        ...
        self.ally_detail_text = scrolledtext.ScrolledText(
        ...
        self.ally_detail_text.pack(fill="x", padx=10, pady=(0, 5))

        self.ally_log_group = tk.LabelFrame(
            self.tab_ally_detail,
            text=f" 📜 近 {MAX_SPECIFIC_LOG_LINES} 次聯盟相關事件 LOG ",
            bg=COLOR_BG_MAIN,
            fg=COLOR_FG_ACCENT,
            font=FONT_UI_BOLD
        )
        self.ally_log_group.pack(fill="both", expand=True, padx=10, pady=(0, 10))
        ...
        self.ally_log_text.pack(fill="both", expand=True, padx=5, pady=5)
    ...
    def append_rich_log_line_to_widget(self, target_widget, line):
        ...
        if any(tag in line for tag in (
            ...
            prefix_tag = "tag_war"
        elif "[出兵征戰]" in line: prefix_tag = "tag_march"
        elif "[戰爭拆屋]" in line: prefix_tag = "tag_destroy"
        elif "[擊破滅亡]" in line: prefix_tag = "tag_defeat"
        elif "[拒絕求和]" in line: prefix_tag = "tag_reject"
        elif "[外交策略]" in line: prefix_tag = "tag_diplomacy"
        elif "[全球事件]" in line: prefix_tag = "tag_global"
    ...
    def on_alliance_selected(self, event=None):
        ...
        members = "、".join(a.get("members", [])) if a.get("members") else "無成員"
        ...
        info_str = f"""
==================================================
 🛡️ {entity_type_text}：{selected_name}
==================================================
• 總 戰 力 ：{a.get("total_power", 0):,}
• 成員數量 ：{a.get("alive_member_count", 0)} / {a.get("member_count", 0)} (存活/總數)
• 圍剿目標 ：{target}
• 聯盟成員 ：
  {members}
==================================================
"""
        if self.ally_detail_text.get("1.0", tk.END).strip() != info_str.strip():
            try:
                old_y = self.ally_detail_text.yview()[0]
            except Exception:
                old_y = 0.0
            self.ally_detail_text.delete("1.0", tk.END)
            self.ally_detail_text.insert(tk.END, info_str)
            self.apply_entity_link_tags(self.ally_detail_text)
            if not alliance_changed:
                try:
                    self.ally_detail_text.yview_moveto(old_y)
                except Exception:
                    pass
        self.load_specific_logs(self.ally_log_text, selected_name, force_top=alliance_changed)
    ...
    def on_country_selected(self, event=None):
        ...
        attackers = "、".join(c.get("recent_attackers", [])) if c.get("recent_attackers") else "無"
        records = "、".join(map(str, c.get("championship_records", []))) if c.get("championship_records") else "無記錄"
...
==================================================
【 🏛️ 國家基本狀態 】
• 國家狀態 ：{"🟢 存活" if c.get("is_alive", True) else "🔴 滅亡"}
• 發展世代 ：第 {c.get("gen", 1)} 代
• 綜合戰力 ：{c.get("power", 0):,}
• 惡名指數 ：{c.get("infamy", 0)}

【 👥 人口與建築 】
• 房屋：{c.get("houses", 0)} 間 (可容納 {c.get("houses", 0) * 10} 人)
• 人口：{c.get("pop_total", 0):,} 人 
• 農夫：{c.get("farmers", 0):,} 人 
• 士兵：{c.get("soldiers", 0):,} 人
• 最近人口稽核：{c.get("last_population_audit", "尚無異常人口變動")}
...
【 ⚔️ 戰績與外交關係 】
• 戰爭狀態：{c.get("war_status", "和平")}
• 參戰編號：{"、".join(c.get("current_wars", [])) if c.get("current_wars") else "無"}
• 普通聯盟：{c.get("current_alliance") if c.get("current_alliance") else "無聯盟 (單打獨鬥)"}
• 反霸權聯軍：{c.get("current_coalition") if c.get("current_coalition") else "未參與"}
• 歷史戰爭：{c.get("wars_fought", 0)} 場（勝 {c.get("wars_won", 0)}／敗 {c.get("wars_lost", 0)}）
• 攻陷國都：{c.get("capital_captures", 0)} 次
• 吞併國家：{c.get("annexed_count", 0)} 個
• 最高仇恨目標：{c.get("top_hostility_target", "無")}
• 奪冠次數：{c.get("championship_count", 0)} 次
• 歷史奪冠紀錄：{records}
• 近期攻擊來源：{attackers}
==================================================
"""
        if self.detail_text.get("1.0", tk.END).strip() != info_str.strip():
            try:
                old_y = self.detail_text.yview()[0]
            except Exception:
                old_y = 0.0
            self.detail_text.delete("1.0", tk.END)
            self.detail_text.insert(tk.END, info_str)
            self.apply_entity_link_tags(self.detail_text)
            if not country_changed:
                try:
                    self.detail_text.yview_moveto(old_y)
                except Exception:
                    pass

        # 國家專屬 LOG：除了國家本身，也納入與該國直接相關的聯盟事件。
        # 例如：該國所屬聯盟發起圍剿、聯盟分裂，或該國成為某反霸權聯盟的討伐目標，
        # 即使該行 LOG 沒有直接寫出國名，也會出現在這個國家的 LOG 中。
        related_alliances = self.get_country_related_alliances(selected_name)
        war_terms = [
            t for t in (c.get("war_log_terms", []) or [])
            if t
        ]
        country_log_terms = (
        ...
        self.load_specific_logs(self.country_log_text, country_log_terms, force_top=country_changed)
