...
"""
    ...
    def on_country_selected(self, event=None):
        ...
        war_state_text = "🔴 戰爭中" if war_info["is_at_war"] else "🟢 和平"
        declared_targets = "、".join(war_info["offense"]) if war_info["offense"] else "無"
        aggressors = "、".join(war_info["defense"]) if war_info["defense"] else "無"
        war_detail_text = (
            "\n".join(war_info["lines"])
            if war_info["lines"]
            else "• 目前沒有進行中的正式戰爭"
...
• 軍事人口比：{(c.get("soldiers", 0) / max(1, c.get("pop_total", 0)) * 100):.1f}%
• 最近人口稽核：{c.get("last_population_audit", "尚無紀錄")}
...
• 歷史參戰：{c.get("wars_fought", 0)} 場
• 戰爭勝敗：勝 {c.get("wars_won", 0)}／敗 {c.get("wars_lost", 0)}
