...
class GameInterfaceUI:
    ...
    def load_specific_logs(self, target_widget, search_terms):
        """載入專屬 LOG。search_terms 可傳單一字串或多個關聯詞。

        只要一行 LOG 命中任一搜尋詞就顯示，並自動去重。
        game.log 本身是「最新在前」，因此直接取前 N 筆，確保真的是最近事件。
        """
        ...
        if os.path.exists("game.log") and terms:
            try:
                ...
                if matched_lines:
                    ...
                    target_widget.insert(tk.END, "（無相關事件紀錄）\n")
            except Exception:
                target_widget.insert(tk.END, "（無法讀取日誌檔案）\n")
...
"""
        ...
        self.load_specific_logs(self.ally_log_text, selected_name)
...
"""
        ...
        self.load_specific_logs(self.country_log_text, country_log_terms)
