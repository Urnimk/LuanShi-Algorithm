...
def generate_awesome_alliance_name(is_anti_hegemon=False, target_name=""):
    ...
    if is_anti_hegemon:  # 若為圍剿霸權的反霸權同盟
        return f"【<<{random.choice(prefixes)}{random.choice(suffixes)}>>】"  # 使用特定括號風格 << >>
...
def get_country_alliance(country_name, alliances):
    """查詢某國家目前所屬的聯盟名稱"""
    for ally_name, members in alliances.items():  # 遍歷所有聯盟與成員清單
        if country_name in members:  # 若該國名在成員清單中
            return ally_name  # 回傳所屬聯盟名稱
    return ""  # 若未加入任何聯盟，回傳空字串
...
def update_rts_economy_and_jobs(data):
    """機器人（AI）自動配比農民/士兵，進行生產、人口增長、蓋房擴建、消耗與轉職"""
    if not data.get("is_alive", True):  # 若國家已滅亡，跳過更新
        return

    c_name = data.get("display_name", "未命名國家")

    # 全域硬限制：每次經濟更新一開始先修正軍民比例，避免舊存檔或其他事件留下非法狀態。
    enforce_farmer_majority(data)

    # 讀取國家各項 RTS 資源數據（提供預設值防止 KeyError）
    houses = max(1, data.get("houses", 15))
    pop_total = max(1, data.get("pop_total", 10))
    food = data.get("food", 300)
    wood = data.get("wood", 200)
    metal = data.get("metal", 100)
    ...
    house_wood_cost = 150  # 建造每間房屋所需的木材費用
    pop_max = houses * 10  # 房屋決定人口上限（每間房提供 10 人口）

    # 蓋房擴建邏輯：若木材足夠且人口快達上限（>=80%）
    if wood >= house_wood_cost and pop_total >= int(pop_max * 0.8):
        affordable_houses = wood // house_wood_cost  # 計算木材可蓋的房屋數量
        build_count = min(
            affordable_houses, random.randint(1, 2)
        )  # 隨機蓋 1~2 間房

        if build_count > 0:
            houses += build_count  # 增加房屋數量
            wood -= build_count * house_wood_cost  # 扣除木材資源
            pop_max = houses * 10  # 更新人口上限
            # print(
            #     f"[經濟擴建]【{c_name}】消耗了 {build_count * house_wood_cost} 木材，新建了 {build_count} 棟房屋！（當前房屋數：{houses}，人口上限：{pop_max}）"
            # )
    ...
    # 人口自然增長邏輯：食物足夠且未達人口上限
    food_cost_per_birth = 20  # 生育一人所需的食物費用
    if food >= food_cost_per_birth and pop_total < pop_max:
        housing_space = pop_max - pop_total  # 剩餘住房空間
        affordable_births = food // food_cost_per_birth  # 糧食可負擔的人口
        growth = min(
            housing_space, affordable_births, random.randint(15, 25)
        )  # 計算實際新增人口

        if growth > 0:
            pop_total += growth  # 增加人口
            food -= growth * food_cost_per_birth  # 扣除消耗食物
            # print(
            #     f"[人口增長]【{c_name}】消耗了 {growth * food_cost_per_birth} 糧食，誕生了 {growth} 名新居民！（總人口：{pop_total}）"
            # )

    # 人口分配與軍民轉職邏輯
    # soldiers = min(soldiers, pop_total)  # 士兵數不可超過總人口
    # farmers = max(0, pop_total - soldiers)  # 剩餘人口皆為農民（勞動力）
    soldiers = min(int(data.get("soldiers", 0) or 0), pop_total // 2)
    # 新生人口預設為農夫，並以 pop_total 為唯一人口基準。
    farmers = pop_total - soldiers
    ...
    # 評估單輪糧食消耗基準（農民 1 單位，士兵 2 單位）
    estimated_consumption = (farmers * 1) + (soldiers * 2)
    SAFETY_TURNS = 3  # 安全存糧門檻（少於 3 輪消耗即判定為糧食不足）

    # 判定是否觸發緊急糧食危機
    is_famine_warning = food < (estimated_consumption * SAFETY_TURNS)

    if is_famine_warning:
        # print(
        #     f"⚠️ [糧食預警]【{c_name}】糧食儲備僅剩 {food}（預計單輪消耗 {estimated_consumption}），不足維持 {SAFETY_TURNS} 輪！緊急啟動全民皆農模式！"
        # )

        # 【緊急戰時警報】：開啟全民皆農模式
        max_allowed_soldiers = int(pop_total * 0.1)  # 只留 10% 士兵

        # 若當前士兵超過 10%，多餘士兵強制退伍轉回農民
        if soldiers > max_allowed_soldiers:
            ...
            print(
                f"🚨 [緊急解編]【{c_name}】強制解編 {demoted_soldiers} 名士兵投入農耕（預計單輪消耗 {estimated_consumption}）"
            )

        # 全民皆種田：100% 農民投入食物生產，停止伐木與採礦
        f_food = farmers
        f_wood = 0
        f_metal = 0
    else:
        # 常態勞工分工：50% 種田、30% 伐木、20% 採礦
        f_food = max(1, int(farmers * 0.5))
        f_wood = max(0, int(farmers * 0.3))
        f_metal = max(0, farmers - f_food - f_wood)

    # 產出資源結算
    prod_food = f_food * 9
    prod_wood = f_wood * 3
    prod_metal = f_metal * 4
    ...
    # print(
    #     f"[資源生產]【{c_name}】生產結算：+ {prod_food} 糧食 ({f_food}人), + {prod_wood} 木材 ({f_wood}人), + {prod_metal} 金屬 ({f_metal}人)"
    # )
    ...
    # 軍事政策由 AI Action 長期控制，而不是只要糧食 > 500 就自動衝到 50% 士兵。
    ...
    if is_famine_warning:
        ...
        if soldiers > desired_soldiers:
            ...
            if metal >= metal_cost_per_soldier and farmers > 0:
                affordable = int(metal // metal_cost_per_soldier)
                batch_limit = random.randint(8, 18)
                max_train_without_breaking_rule = max(0, (farmers - soldiers) // 2)
                train_count = min(
                    needed, farmers, affordable, batch_limit, max_train_without_breaking_rule
                )

                if train_count > 0:
                    soldiers += train_count
                    farmers -= train_count
                    metal -= train_count * metal_cost_per_soldier

    # ---------------------------------------------------------
    # 🍞 5. 維持費與飢荒扣減
    # ---------------------------------------------------------

    # 消耗糧食結算 (農民 1 單位，士兵 2 單位)
    base_food_consumed = (farmers * 1) + (soldiers * 2)

    if pop_total > pop_max:  # 若人口超載（房屋不足）
        overcrowded_pop = pop_total - pop_max  # 超載人口數量
        extra_food_consumed = overcrowded_pop * 1  # 懲罰性額外食物消耗
        food_consumed = base_food_consumed + extra_food_consumed
        # print(
        #     f"🏚️ [住房超載]【{c_name}】住房不足，產生懲罰性消耗 {extra_food_consumed} 糧食！"
        # )
    else:
        food_consumed = base_food_consumed

    food -= food_consumed  # 扣除總消耗食物
    # print(
    #     f"[糧食消耗]【{c_name}】本輪共消耗 {food_consumed} 糧食（剩餘糧食：{food}）"
    # )

    # 軍隊金屬維護費
    metal_maintenance = int(soldiers * 0.5)  # 每名士兵消耗 0.5 金屬維護費
    if metal >= metal_maintenance:
        metal -= metal_maintenance  # 金屬足夠，直接扣除
    else:  # 金屬不足以養兵，部分士兵被迫退伍
        maintained_soldiers = int(metal // 0.5)  # 能維持的士兵數
        demoted = soldiers - maintained_soldiers  # 退伍數
        soldiers = max(0, maintained_soldiers)
        farmers = max(0, pop_total - soldiers)  # 轉回農民
        metal = 0  # 金屬耗盡
        # print(
        #     f"💸 [軍費不足]【{c_name}】金屬不足支付軍費，{demoted} 名士兵被迫退伍轉為農民！"
        # )

    # 饑荒懲罰機制 (食物小於 0 時人口餓死)
    if food < 0:
        starved = min(
            pop_total, math.ceil(abs(food) / 2)
        )  # 根據欠糧程度計算餓死人數
        food = 0  # 食物清零
        pop_total = max(1, pop_total - starved)  # 扣除餓死人口（最低留 1 人）
        soldiers = min(soldiers, pop_total)  # 修正士兵數
        farmers = max(0, pop_total - soldiers)  # 修正農民數
        print(
            f"☠️ [爆發饑荒]【{c_name}】糧食耗盡！導致 {starved} 名居民餓死！（剩餘人口：{pop_total}）"
        )
    ...
    data["houses"] = houses
    data["pop_total"] = pop_total
    data["soldiers"] = min(max(0, soldiers), pop_total // 2)
    data["farmers"] = pop_total - data["soldiers"]
    data["food"] = max(0, food)
    data["wood"] = max(0, wood)
    data["metal"] = max(0, metal)
...
def get_valid_rl_actions(attacker_key, countries, alliances, anti_hegemon_targets, alive_sorted):
    """依照客觀條件建立 Action Mask，避免 AI 選到根本不可能或明顯無意義的行動。"""
    ...
    if (
        ...
        and len(anti_hegemon_targets) < 2
    ):
        valid.add(2)
    ...
    if wood >= 40 and metal >= 20 and c.get("fortification", 0) < 95:
        valid.add(5)
...
def thread_stat_monitor():
    ...
    num = 100  # 初始生成國家數量
    coalition_size_min = 4  # 反霸權圍剿聯盟最小規模
            ...
            if saved_countries:
                ...
                for base_name, c_data in countries.items():
                    ...
                    if isinstance(c_data.get("power"), dict):
                        ...
                        c_data["food"] = p_dict.get("food", 2000)
                        c_data["wood"] = p_dict.get("wood", 500)
                        c_data["metal"] = p_dict.get("metal", 500)
                ...
                anti_hegemon_targets = saved_alliances_data.get("anti_hegemon_targets", {})
    ...
    if not is_loaded:
        ...
        for name in base_country_names:
            ...
            farmers = int(pop * 0.6)  # 60% 農民
            soldiers = pop - farmers  # 40% 士兵
            ...
            countries[name] = {  # 設定國家初始屬性
                ...
                "farmers": farmers,
                "soldiers": soldiers,
                "food": random.randint(1500, 3000),
                "wood": random.randint(300, 800),
                "metal": random.randint(300, 800),
                "power": 0,
                "is_alive": True,
                "annexed_count": 0,
    ...
    while True:
        ...
        for ally_name, members in list(alliances.items()):
            ...
            for m in alive_m:
                ...
                if is_overpowered:  # 若國家戰力太大，原有非圍剿同盟會自動瓦解（避免大國抱團）
                    is_anti_hegemon_ally = ally_name.startswith("【極限討伐】") or ally_name.startswith("【霸權圍剿】")
                    if not is_anti_hegemon_ally:
                        print(
            ...
            # 若同盟整體戰力佔全球 10% 以上或過半，引發同盟內鬥解散
            if ally_name not in alliances_to_remove and not ally_name.startswith("【極限討伐】"):
        ...
        for ally_n in set(alliances_to_remove):
            ...
            if ally_n in anti_hegemon_targets:
                del anti_hegemon_targets[ally_n]
        ...
        if event_msg:
            ...
            if event_type == "BOOM":  # 大發掘：全體獲取物資
                for k in alive_sorted:
                    countries[k]["metal"] += random.randint(150, 300)
                    countries[k]["food"] += random.randint(200, 400)
        ...
        if len(alive_sorted) >= 2:
            ...
            for attacker_key in active_countries:
                ...
                valid_actions = get_valid_rl_actions(
                    attacker_key, countries, alliances, anti_hegemon_targets, alive_sorted
                )
                ...
                if action == 0:
                    att_entity = get_entity_info(attacker_key, countries, alliances)
                    possible_defenders = [
                        k for k in alive_sorted
                        if k not in att_entity["members"]
                    ...
                    if possible_defenders:
                        action_success = True
                        defender_key = None
                        curr_ally = c_data.get("current_alliance")

                        # 討伐目標優先攻擊霸權
                        if curr_ally in anti_hegemon_targets:
                            target_hegemon = anti_hegemon_targets[curr_ally]
                            if target_hegemon in possible_defenders and random.random() < 0.2:
                                defender_key = target_hegemon
                        ...
                        # if one == 1 :
                            ...
                            tribute = max(30, int(def_pwr * 0.2))  # 轉移糧食作為歲幣
                            tribute = min(tribute, def_pwr)

                            if accept_peace and tribute > 0:  # 接受割地求和
                                
                                
                                # 防守方支付歲幣
                                if def_entity["type"] == "SINGLE":
                                    countries[defender_key]["food"] = max(0, countries[defender_key]["food"] - tribute)
                                else:
                                    for m in def_entity["members"]:
                                        countries[m]["food"] = max(0, countries[m]["food"] - int(tribute / len(def_entity["members"])))

                                # 進攻方收取歲幣
                                if att_entity["type"] == "SINGLE":
                                    c_data["food"] += tribute
                                else:
                                    for m in att_entity["members"]:
                                        countries[m]["food"] += int(tribute / len(att_entity["members"]))
                        ...
                        war_tactics = random.choice([0, 1, 2])  # 隨機選擇戰術類型 (0:拆屋, 1:屠殺, 2:掠奪)
                        att_soldiers = int(sum(countries[m]["soldiers"] for m in att_entity["members"]))
                        
                        # 補給消耗
                        war_food_cost = att_soldiers * 2
                        war_metal_cost = att_soldiers * 5

                        c_data["food"] = max(0, c_data.get("food", 0) - war_food_cost)
                        c_data["metal"] = max(0, c_data.get("metal", 0) - war_metal_cost)

                        print(f"[出兵征戰]【{att_entity['name']}】出兵 {att_soldiers} 人，消耗了 {war_food_cost} 糧草與 {war_metal_cost} 兵器！")
                        
                        # war_tactics = 2

                        if war_tactics == 0:  # 戰術 0：拆毀敵方房屋
                            max_destroy_limit = max(1, att_soldiers // 10)
                            total_destroyed = 0
                            ...
                            for def_m in def_entity["members"]:
                                ...
                                if def_soldiers > 0:
                                    # 1. 交戰規模以兵力為主
                                    clash_size = int(
                                        min(att_soldiers, def_soldiers) * random.uniform(0.3, 0.6)
                                    )
                                    ...
                                    raw_kills = int(
                                        clash_size * random.uniform(0.8, 1.1) / defense_factor
                                    )
                                    ...
                                    soldier_kills = 0
                                    farmer_kills = min(
                                        def_farmers, int(att_soldiers * random.uniform(0.1, 0.3))
                                    )
                                    farmer_kills = (
                                ...
                                # 扣除攻擊方戰損
                                c_data["soldiers"] = max(0, c_data["soldiers"] - att_loss)
                                c_data["pop_total"] = c_data["soldiers"] + c_data["farmers"]
                                enforce_farmer_majority(c_data)
                                c_data["power"] = calculate_rts_power(c_data)
                                total_att_losses += att_loss
                            ...
                            max_f = soldiers_for_f * 19
                            max_m = soldiers_for_m * 19
                            max_w = soldiers_for_w * 11

                            for def_m in def_entity["members"]:
                                # 4. 一次拉取，不超過敵方存量與士兵負重上限
                                stolen_f = min(countries[def_m]["food"], max_f)
                                stolen_m = min(countries[def_m]["metal"], max_m)
                                stolen_w = min(countries[def_m]["wood"], max_w)
                                ...
                                c_data["food"] += stolen_f
                                c_data["metal"] += stolen_m
                                c_data["wood"] += stolen_w
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                        ...
                        if def_destroyed:
                            for dm in def_destroyed:
                                if countries[dm]["is_alive"]:
                                    ...
                                    countries[dm]["death_respawn_cooldown"] = 3  # 設定復活冷卻回合

                            # 勝者獲得額外戰利品
                            loot1, loot2, loot3 = random.randint(300, 800), random.randint(300, 800), random.randint(300, 800)
                            c_data["food"] += loot1
                            c_data["wood"] += loot2
                            c_data["metal"] += loot3

                            c_data["annexed_count"] += 1  # 吞併國家數 +1
                            action_value += 500.0
                            print(f"[擊破滅亡] 【{att_entity['name']}】成功擊潰【{def_entity['name']}】並掠奪物資 + 糧草:{loot1} / 金屬:{loot3} / 木材:{loot2} ！")
                ...
                # ----------------------------------------------------
                # Action 2: 號召極限圍剿 (反霸權聯盟)
                # ----------------------------------------------------  
                elif action == 2:
                    current_anti_hegemon_count = len(anti_hegemon_targets)
                    coal_max = 2  # 全球上限最多同時存在 2 個圍剿聯盟
                    
                    if current_anti_hegemon_count < coal_max:
                        hegemon_key = alive_sorted[0]  # 目標指定為第一名霸主
                        if hegemon_key != attacker_key and countries[hegemon_key]["is_alive"]:
                            hegemon_allies = alliances.get(countries[hegemon_key]["current_alliance"], [])
                            # 篩選非霸權且非霸權盟友的國家作為候選人
                            candidates = [k for k in alive_sorted if k != hegemon_key and k not in hegemon_allies and countries[k]["is_alive"]]
                            
                            if len(candidates) >= 3:
                                coal_size = min(coalition_size_min, len(candidates))
                                coal_members = random.sample(candidates, coal_size)
                                if attacker_key not in coal_members:
                                    coal_members.append(attacker_key)

                                coal_name = generate_awesome_alliance_name(
                                    is_anti_hegemon=True,
                                    target_name=countries[hegemon_key]["display_name"],
                                )

                                # 將候選成員退出原同盟並加入圍剿同盟
                                for m in coal_members:
                                    curr_a = countries[m]["current_alliance"]
                                    if curr_a in alliances and m in alliances[curr_a]:
                                        alliances[curr_a].remove(m)
                                    countries[m]["current_alliance"] = coal_name

                                alliances[coal_name] = coal_members
                                anti_hegemon_targets[coal_name] = hegemon_key
                                action_success = True
                                print(f"[極限圍剿] 【{c_data['display_name']}】號召 {len(coal_members)} 國組成 {coal_name} 圍剿霸權！（圍剿聯盟：{len(anti_hegemon_targets)}/{coal_max}）")

                # Action 3: 養精蓄銳 (發展經濟)
                elif action == 3:
                    multiplier = 2.0 if event_type == "EXPANSION" else 1.0  # 大航海時代產量翻倍
                    growth_food = int(random.randint(100, 200) * multiplier)
                    growth_wood = int(random.randint(80, 150) * multiplier)
                    c_data["food"] += growth_food
                    c_data["wood"] += growth_wood
                    c_data["metal"] += int(random.randint(30, 70) * multiplier)
                    action_success = True
                    action_value += growth_food + growth_wood
                    print(f"[養精蓄銳] 【{c_data['display_name']}】深耕經濟與採集，資源存量提升！")
                ...
                # ----------------------------------------------------                
                    ...
                    if curr_a and curr_a in alliances:
                        ...
                        if partners:
                            ...
                            steal_food = int(countries[victim]["food"] * 0.3)  # 偷取對方 30% 食物
                ...
                # ----------------------------------------------------
                    ...
                    wood_cost = min(c_data["wood"], random.randint(40, 90))
                    metal_cost = min(c_data["metal"], random.randint(20, 55))
                    if wood_cost >= 40 and metal_cost >= 20:
                ...
                # ----------------------------------------------------
                    ...
                    train_count = min(need, safe_train, affordable, random.randint(5, 18))
                    if train_count > 0:
                        c_data["soldiers"] += train_count
                        c_data["farmers"] -= train_count
                        c_data["metal"] -= train_count * 12
                        c_data["food"] -= train_count * 5
                        action_success = True
                ...
                # ----------------------------------------------------
                    ...
                    investment = min(c_data["wood"], random.randint(15, 45))
                    c_data["wood"] -= investment
                    food_gain = int(farmers * random.uniform(2.0, 3.5) + investment * 2.0)
                ...
                # ----------------------------------------------------
                    ...
                    if food_turns < 3 and (c_data["wood"] >= 30 or c_data["metal"] >= 20):
                        ...
                        gain = int(sell_wood * 1.2 + sell_metal * 2.2)
                        c_data["food"] += gain
                        action_success = True
                        action_value += gain
                ...
                # ----------------------------------------------------
                    ...
                    if old_infamy > 0:
                        diplomacy_cost = min(c_data["food"], random.randint(30, 80))
                ...
                # ----------------------------------------------------
                    ...
                    if raid_targets and c_data["soldiers"] >= 5:
                        ...
                        t = countries[target]
                        raid_force = min(c_data["soldiers"], max(5, int(c_data["soldiers"] * 0.15)))
                        food_cost = raid_force
                        c_data["food"] = max(0, c_data["food"] - food_cost)

                        steal_f = min(t["food"], max(10, int(t["food"] * 0.05)))
                        steal_w = min(t["wood"], max(5, int(t["wood"] * 0.04)))
                        steal_m = min(t["metal"], max(5, int(t["metal"] * 0.04)))
                        t["food"] -= steal_f
                        t["wood"] -= steal_w
                        t["metal"] -= steal_m
                        c_data["food"] += steal_f
                        ...
                        # 襲擾仍有低比例戰損與外交代價
                        loss = min(c_data["soldiers"], int(raid_force * random.uniform(0.02, 0.08)))
                ...
                # ----------------------------------------------------
                    ...
                    if soldiers > 0:
                        ...
                        # 退役不退還全部軍費，只回收少量可用裝備。
                        c_data["metal"] += int(demobilize * 0.8)
                        action_success = True
            ...
            if not data["is_alive"]:
                if dead_mode == 0:  # 冷卻模式
                    if data["death_respawn_cooldown"] > 0:
                        ...
                        data["farmers"] = 7
                        data["soldiers"] = 3
                        data["food"] = random.randint(200, 400)
                        data["wood"] = random.randint(150, 300)
                        data["metal"] = random.randint(80, 200)
                        enforce_farmer_majority(data)
                        data["power"] = calculate_rts_power(data)
        ...
        # 7. 討伐目標瓦解與使命完成檢查
        for ally_name in list(anti_hegemon_targets.keys()):
            target_id = anti_hegemon_targets[ally_name]
            target_c = countries.get(target_id)

            if ally_name not in alliances:  # 聯盟已消失
                del anti_hegemon_targets[ally_name]
                continue

            # 目標國家已覆滅，解除圍剿任務並解散聯盟
            if not target_c or not target_c.get("is_alive", True):
                print(f"[使命完成] 討伐目標【{target_c['display_name'] if target_c else '霸權'}】已覆滅！討伐聯軍 【{ally_name}】 解散！")
                if ally_name in alliances:
                    del alliances[ally_name]
                del anti_hegemon_targets[ally_name]
        ...
        for ally_name, members in alliances.items():
            ...
            alliance_details[ally_name] = {
                ...
                "members": members,
                "target_hegemon": anti_hegemon_targets.get(ally_name, None),
            }
                ...
                json.dump(
                    {
                        ...
                        "anti_hegemon_targets": anti_hegemon_targets,
                    },
                    f_a,
                    ensure_ascii=False,
                    indent=4,
