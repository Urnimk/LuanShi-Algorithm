...
from 亂世演算_V8_4_9_決策分析 import NationalDecisionHub
...
ENGINE_VERSION = "亂世演算V8_4_11_動態交易貨幣輪替版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
MAX_NUMBER = 20 #參與國家數量
...
def _infrastructure_investment_cost(data):
    ...
    return (
        INFRASTRUCTURE_GOLD_BASE + tier * 20 * MONEY_RATIO,
...
def get_valid_rl_actions(attacker_key, countries, alliances, anti_hegemon_coalitions, alive_sorted):
    ...
    if c.get("infamy", 0) > 0 and gold >= 60 * MONEY_RATIO:
        valid.add(9)
...
class CountryRLAgent:
    ...
    def _get_strategic_state(self, country_data, total_world_power, max_power, alive_count):
        ...
        policy_floor = min(BIRTH_POLICY_GOLD_COST, PRODUCTION_POLICY_GOLD_COST)
        ...
        return NationalDecisionHub.strategic_state(existing_state, country_data, report)
...
def thread_stat_monitor():
    ...
    while True:
        ...
        for data in countries.values():
            ...
            data["cash_status"] = (
                "無政策預算" if cash < min(BIRTH_POLICY_GOLD_COST, PRODUCTION_POLICY_GOLD_COST)
                else "可執行政策" if cash < min(BIRTH_POLICY_GOLD_COST, PRODUCTION_POLICY_GOLD_COST) * 3
                else "資金充裕"
            )
        ...
        if len(alive_sorted) >= 2:
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # ----------------------------------------------------
                    ...
                    if old_infamy > 0 and gold_stock >= 60 * MONEY_RATIO:
                        min_cost = 60 * MONEY_RATIO
                        max_cost = min(gold_stock, max(80 * MONEY_RATIO, int(gold_stock * 0.16)))
                        ...
                        reduction = random.randint(14, 28) + int(min(10, diplomacy_cost / (20 * MONEY_RATIO)))
