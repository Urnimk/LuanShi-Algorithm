...
# 政權更替採極低機率；政治穩定度崩壞時才會顯著上升。
REGIME_CHANGE_COOLDOWN_TURNS = 90
SUCCESSION_MIN_TENURE = 160
SUCCESSION_CHANCE_PER_COUNTRY = 0.00003
...
def _unique_regime_name(country_key, countries, suffixes):
    """政變改名時，新國號前兩字必須跟原始國家 key 前兩字完全相同。"""
    base = str(country_key).strip()
    prefix = base[:2] if len(base) >= 2 else base
    used = {
        str(c.get("display_name", k)).strip()
        for k, c in countries.items()
        if k != country_key
    }
    choices = list(suffixes)
    random.shuffle(choices)
    gen = int(countries[country_key].get("gen", 1) or 1)
    for suffix in choices:
        candidate = f"{prefix}{suffix}"
        if candidate not in used:
            return candidate
    # 極端重名時仍保證前兩字不變。
    return f"{prefix}{random.choice(choices)} {to_roman(max(2, gen))}"
    ...
    history.append(
        {
            ...
            "generation": int(data.get("gen", 1) or 1),
        }
    )
...
def _apply_regime_change(
    ...
    old_gen = int(data.get("gen", 1) or 1)
    data["gen"] = old_gen + 1
    data["regime_tenure"] = 0
    data["last_regime_change_turn"] = int(turn_counter)
    data["regime_change_count"] = int(data.get("regime_change_count", 0) or 0) + 1

    agent = rl_agents.get(country_key)

    if kind == "SUCCESSION":
        # 正統繼承：世代 +1，但國號與基本國策連續。
        new_name = old_name
        ...
        data["regime_type"] = data.get("regime_type", "正統政權") or "正統政權"
        event_text = f"正統繼承：第 {data['gen']} 代完成交接，國號維持【{new_name}】"
        _set_last_event(data, event_text)
        _record_regime_history(data, turn_counter, "正統繼承", old_name, new_name)
        print(f"👑 [正統繼承]【{old_name}】完成第 {data['gen']} 代交接，國號不變。")
        return True
    ...
    print(
        f"⚠️ [{kind_name}]【{old_name}】政權更替，第 {data['gen']} 代改國號為【{new_name}】；"
        f"國家本體仍為「{country_key}」。"
    )
    return True
...
def _war_round_upkeep(members, countries, war_id, active_wars):
    ...
    for m in _war_live_members(members, countries):
        ...
        need_food = int(math.ceil(effective * WARSTATE["WAR_ROUND_FOOD_PER_SOLDIER"]))
        need_metal = int(math.ceil(effective * WARSTATE["WAR_ROUND_METAL_PER_SOLDIER"]))
...
def _war_set_truce_between_sides(attacker_members, defender_members, war_truces, turn_counter):
    expiry = turn_counter + WARSTATE["TRUCE_TURNS"]
...
def thread_stat_monitor():
    ...
    if os.path.exists(countries_file) and os.path.exists(alliances_file):
        try:
            ...
            if saved_countries:
                ...
                loaded_season_elapsed = float(saved_alliances_data.get("season_elapsed_seconds", 0.0) or 0.0)
                loaded_world_event_counter = int(saved_alliances_data.get("dynamic_event_counter", 0) or 0)
    ...
    if is_loaded and loaded_season_elapsed > 0:
        start_time = time.time() - min(loaded_season_elapsed, float(time_total))
        ...
        if event_msg:
            ...
            if event_type == "BOOM":  # 大發掘：全體獲取物資
                ...
                for k in alive_sorted:
                    ...
                    new_pop = max(1, int(old_pop * 0.95))
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # ---------------------------------------------------- 
                    ...
                    if not c_data["current_alliance"]:  # 僅在尚未加入同盟時執行
                        ...
                        # 50% 嘗試加入現有同盟，50% 嘗試創建新同盟。
                        # 加盟不再是無條件成功：惡名越高，既有盟友越可能拒絕。
                        if existing_alliances and random.random() < 0.5:
                            target_ally = random.choice(existing_alliances)
                            join_chance = 0.72 - min(0.42, c_data.get("infamy", 0) / 180.0)
                            if random.random() < max(0.20, join_chance):
                                alliances[target_ally].append(attacker_key)
                                alliances[target_ally] = list(dict.fromkeys(alliances[target_ally]))
                                c_data["current_alliance"] = target_ally
                                action_success = True
                                print(
                                    f"[外交策略] 【{c_data['display_name']}】獲准加入 {target_ally}！"
                                    f"（成員數：{len(alliances[target_ally])}）"
                                )
                            else:
                                print(
                                    f"[外交受挫] 【{c_data['display_name']}】申請加入 {target_ally}，"
                                    f"但因信任不足遭拒。"
                                )
                        else:  # 尋找未結盟國家創立新同盟
                            unallied = [
                ...
                # ----------------------------------------------------
                    ...
                    recruit_event_mult = float(c_data.get("event_recruit_mult", 1.0) or 1.0)
                    mobilize_cap = min(
                ...
                # ----------------------------------------------------
                    ...
                    trade_event_mult = float(c_data.get("event_trade_mult", 1.0) or 1.0)
            ...
            if not data["is_alive"]:
                if dead_mode == 0:  # 冷卻模式
                    if data["death_respawn_cooldown"] > 0:
                        ...
                        old_regime_name = data.get("display_name", name)
                        data["display_name"] = f"{name} {gen_str}"  # 例如：挫蛋先鋒 II
                        data["is_alive"] = True  # 復活
                        _record_regime_history(
                            data, turn_counter, "亡國重建", old_regime_name, data["display_name"]
                        )
        ...
        try:
            ...
            _atomic_write_json(
                "war_live_alliances.json",
                {
                    ...
                    "season_elapsed_seconds": min(
                        float(time_total), max(0.0, time.time() - start_time)
                    ),
                    "engine_version": "戰國模式V3_事件政變版",
                },
                compact=True,
