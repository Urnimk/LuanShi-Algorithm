...
ENGINE_VERSION = "亂世演算V8_5_5_供需貨幣穩定版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
def _market_health_update(market, bank, countries, turn_counter):
    ...
    for resource in MARKET_RESOURCES:
        ...
        if qty >= MARKET_MONITOR_MIN_RESOURCE_QTY and value > 0:
            ...
            interval_prices[resource] = round(normalized_vwap, 6)
    ...
    observation = {
        "turn": int(turn_counter), "index": round(index, 6) if index is not None else None,
...
def _write_rl_monitoring(turn_counter, countries, agents, world_market):
    ...
    flat = {key: value for key, value in record.items() if key not in
            ("strategic_actions", "market_price_index", "market_price_index_valid",
             "market_10turn_volume", "market_diagnosis")}
