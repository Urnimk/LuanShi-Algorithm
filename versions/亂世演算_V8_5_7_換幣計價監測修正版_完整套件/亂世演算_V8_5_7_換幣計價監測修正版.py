...
ENGINE_VERSION = "亂世演算V8_5_7_換幣計價監測修正版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
def _market_place_layered_order(country_key, countries, market, turn_counter, market_action, price_action, size_action):
    ...
    price = round(_clamp(price, max(_market_price_floor(resource, market), reference * 0.80), min(_market_price_ceiling(resource, market), reference * 1.20)), 2)
    pop = max(1, int(data.get("pop_total", 1) or 1))
