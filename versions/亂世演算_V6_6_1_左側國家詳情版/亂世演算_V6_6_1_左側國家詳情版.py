...
def _coup_probability(data, turn_counter):
    ...
    # | 政治穩定度  | 基礎政變率／年 | 約等於   
    # | ------     | ------:       | ----------
    # | 65～100    | 0.0002%       |  約 50 萬年一次  
    # | 45～64     | 0.0015%       |  約 6.7 萬年一次 
    # | 30～44     |  0.008%       |  約 1.25 萬年一次 
    # | 15～29     |  0.035%       |  約 2,857 年一次  
    # | 0～14      |   0.13%       |  約 769 年一次  
...
def _maybe_trigger_regime_change(
    ...
    for key in shuffled:
        data = countries[key]
        if random.random() >= _coup_probability(data, turn_counter): #機率看這裡
            continue
        ...
        try:
            ...
            _atomic_write_json(
                "war_live_alliances.json",
                {
                    ...
                    "engine_version": "亂世演算V6.6.1_左側國家詳情版",
                },
                compact=COMPACT_FORMAT,
            )
