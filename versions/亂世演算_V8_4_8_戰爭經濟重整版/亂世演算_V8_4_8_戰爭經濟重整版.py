...
ENGINE_VERSION = "亂世演算V8_4_7_戰前軍事風險動員版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
# 市場價格護欄：禁止價格因單邊拋售無限向 0 漂移。
MARKET_PRICE_FLOOR_RATE = 0.60
MARKET_PRICE_CEILING_RATE = 2.50
# 世界銀行護盤：價格跌破種子價一定比例時，用既有金庫吸收賣單。
WORLD_BANK_MARKET_SUPPORT_TRIGGER = 0.3
WORLD_BANK_MARKET_SUPPORT_BUDGET_RATE = 0.15
WORLD_BANK_MARKET_SUPPORT_MAX_PER_RESOURCE = 80_000
WORLD_BANK_MARKET_SUPPORT_PRICE_PREMIUM = 1.05
...
def _world_bank_pay_bounty(bank, country_data, amount, reason="國際懸賞"):
    ...
    bank["last_action"] = f"{reason}：{paid} 金幣"
    return paid
...
def _market_place_layered_order(country_key, countries, market, turn_counter, market_action, price_action, size_action):
    ...
    # 小國保留平方根基礎量；大國另有受控的線性大宗容量。
    # 線性部分最多只讓單筆掛單觸及人口約 12%，避免萬年富國仍只下零售單，
    # 同時不讓人口規模直接壓垮市場。實際數量仍受現金或庫存約束。
    ...
    capacity = max(5, min(int(pop * 0.12), int(retail_capacity + bulk_capacity)))
...
def _world_bank_market_support(bank, countries, market, resource, turn_counter):
    """
    世界銀行逆週期護盤。
    觸發條件：最近價格低於種子價 65%。
    作法：使用銀行既有金庫，直接吃掉低價賣單；實物存入銀行市場庫存。
    不憑空發行金幣，因此仍維持貨幣守恆。
    """
    stat = market.get("stats", {}).get(resource, {}) or {}
    seed = float(MARKET_RESOURCE_META[resource]["seed_price"])
    last_price = max(0.05, float(stat.get("last_price", seed) or seed))
    trigger = seed * WORLD_BANK_MARKET_SUPPORT_TRIGGER
    if last_price >= trigger:
        return 0

    treasury = max(0, int(bank.get("treasury", 0) or 0))
    budget = min(
        int(treasury * WORLD_BANK_MARKET_SUPPORT_BUDGET_RATE),
        int(WORLD_BANK_MARKET_SUPPORT_MAX_PER_RESOURCE),
    )
    if budget <= 0:
        return 0

    support_price = max(_market_price_floor(resource), trigger * WORLD_BANK_MARKET_SUPPORT_PRICE_PREMIUM)
    bought_qty = 0
    spent = 0
    # 只吸收願意以護盤價以下出售的單，避免銀行變成無限量買家。
    sells = sorted(
        [o for o in market.get("orders", [])
         if o.get("resource") == resource and o.get("side") == "SELL"
         and int(o.get("qty", 0) or 0) > 0
         and float(o.get("price", 0) or 0) <= support_price],
        key=lambda o: (float(o.get("price", 0) or 0), int(o.get("turn", 0) or 0)),
    )
    for order in sells:
        ...
        if not seller or not seller.get("is_alive", True):
            order["qty"] = 0
            continue
        available = max(0, int(seller.get(resource, 0) or 0))
        qty = min(int(order.get("qty", 0) or 0), available, int((budget - spent) / max(1, int(round(float(order.get("price", support_price) or support_price))))))
        if qty <= 0:
            continue
        unit_price = max(_market_price_floor(resource), float(order.get("price", support_price) or support_price))
        gross = int(round(qty * unit_price))
        if gross <= 0 or spent + gross > budget:
            continue
        seller[resource] = available - qty
        seller["gold"] = max(0, int(seller.get("gold", 0) or 0)) + gross
        order["qty"] = int(order.get("qty", 0) or 0) - qty
        spent += gross
        bought_qty += qty
        bank.setdefault("market_reserves", {}).setdefault(resource, 0)
        bank["market_reserves"][resource] += qty

    if bought_qty:
        bank["treasury"] = treasury - spent
        bank["last_action"] = f"市場護盤：{MARKET_RESOURCE_META[resource]['name']}買進 {bought_qty} 單位"
        # 護盤成交後，價格至少回到護盤參考價，避免 UI 繼續顯示已失真的極低價格。
        stat["last_price"] = max(float(stat.get("last_price", 0) or 0), support_price)
        hist = list(stat.get("price_history", []) or [])
        hist.append(round(support_price, 2))
        stat["price_history"] = hist[-MARKET_PRICE_HISTORY_MAX:]
        stat["last_trade_turn"] = int(turn_counter)
        stat["last_trade_volume"] = bought_qty
    return bought_qty
...
def _market_match_orders(countries, market, bank, turn_counter):
    ...
    # 正常撮合後再護盤，吸收低於警戒線的過剩賣單。
    for resource in MARKET_RESOURCES:
        _world_bank_market_support(bank, countries, market, resource, turn_counter)
    _market_prune_orders(market, countries, turn_counter)
    return trades
...
class CountryRLAgent:
    ...
    def _get_state_key(self, country_data, total_world_power, max_power, alive_count):
        ...
        # 所有資產先換回「基準金幣」再做人均分級，避免 MONEY_RATIO 放大後
        # 金幣一項獨占經濟狀態，使正常國家幾乎永遠被標成 RICH。
    ...
    def choose_action(
        ...
        # ------------------------------------------------------------
        # 戰略風險反應：戰前就開始準備，而不是等開戰才補兵。
        # prewar_* 由主迴圈依全世界資料計算；Q-learning 也能直接讀到這些狀態。
        # 高風險時只「提高動員行動被選中的機率」，不建立固定人格。
        # 只有極端危險且具備動員條件時，才使用小型硬安全規則避免明顯自殺。
        # ------------------------------------------------------------
        prewar_score = float(country_data.get("prewar_risk_score", 0) or 0)
        prewar_need = float(country_data.get("prewar_mobilization_need", 0) or 0)
        recruit_ready = bool(country_data.get("prewar_recruit_ready", False))
        soldier_ratio = max(0, int(country_data.get("soldiers", 0) or 0)) / max(1, int(country_data.get("pop_total", 1) or 1))
        farmer_soldier_ratio = float(country_data.get("prewar_farmer_soldier_ratio", 0) or 0)
        prewar_mobilize = False

        if (
            6 in engine_valid
            and not country_data.get("current_wars")
            and recruit_ready
            and (prewar_score >= PREWAR_RISK_ALERT or farmer_soldier_ratio >= PREWAR_FARMER_SOLDIER_ALERT_RATIO)
        ):
            # 風險越高、長期軍備缺口越大，越容易提前動員；仍保留 RL 的自主選擇。
            soft_chance = PREWAR_MOBILIZE_SOFT_CHANCE_MIN + (
                PREWAR_MOBILIZE_SOFT_CHANCE_MAX - PREWAR_MOBILIZE_SOFT_CHANCE_MIN
            ) * _clamp(prewar_need, 0.0, 1.0)
            if random.random() < soft_chance:
                prewar_mobilize = True

        # 極端情況：鄰國明顯壓倒、自己兵力低於 22%，又有糧金與可徵人口，
        # 若完全不動員可能下一輪就失去反應時間；此處只做最後一道生存保護。
        critical_safety = (
            6 in engine_valid
            and not country_data.get("current_wars")
            and prewar_score >= PREWAR_RISK_CRITICAL
            and recruit_ready
            and soldier_ratio <= PREWAR_CRITICAL_SOLDIER_RATIO
            and (
                float(country_data.get("prewar_threat_power_ratio", 0) or 0) >= 1.15
                or farmer_soldier_ratio >= PREWAR_FARMER_SOLDIER_CRITICAL_RATIO
            )
        )
        # 即使沒有明確敵人，農民達士兵 4.5 倍以上也代表自身戰備嚴重失衡。
        force_imbalance_safety = (
            6 in engine_valid
            and not country_data.get("current_wars")
            and recruit_ready
            and farmer_soldier_ratio >= PREWAR_FARMER_SOLDIER_CRITICAL_RATIO
        )

        if force_imbalance_safety or critical_safety or prewar_mobilize:
            strategic_action = 2  # 軍備與動員
            engine_action = 6
        else:
...
def _war_collect_reparations(winner_members, loser_members, countries):
    ...
    # 戰敗金：敗方現有金幣的 12%，至少 1；避免打贏卻完全沒有戰果。
    for m in losers:
    ...
    while True:
                ...
                if winner_side == "ATTACKER":
                    ...
                    if war.get("is_coalition_war"):
                        ...
                        print(
                            ...
                            f"世界銀行從現有庫存支付懸賞 {bounty_total} 金幣。"
                        )
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # Action 3: 基礎建設投資（支付現有資產，換取永久效率）
                    ...
                    if investment:
                        ...
                        action_value = gain
                        print(
                            f"[基礎建設] 【{c_data['display_name']}】支付金幣 {investment['gold']}、"
        ...
        if (time.time() - start_time) > time_total:
            if sorted_countries:
                ...
                print(
                    f"💰 [冠軍實體獎勵] 冠軍【{champion_data['display_name']}】獲 {physical_rewards['champion']} 金幣；"
