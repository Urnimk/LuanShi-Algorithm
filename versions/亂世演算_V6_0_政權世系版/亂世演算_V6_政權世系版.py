...
def _infer_country_generation_from_history(data):
    """
    舊 V5.x 存檔轉 V6：
    優先沿用畫面上最後一次真正出現的復國羅馬世代。
    若政變已把羅馬尾碼蓋掉，就往 regime_history 找最後一次亡國重建。
    """
    current = str(data.get("display_name", "") or "")
    parsed = _extract_old_roman_generation(current)
    if parsed > 0:
        return parsed
    ...
    for item in reversed(history):
        if not isinstance(item, dict):
            continue
        if str(item.get("kind", "")) != "亡國重建":
            continue
        parsed = _extract_old_roman_generation(
            item.get("new_name", "")
        )
        if parsed > 0:
            return parsed
...
def _format_regime_display_name(
    ...
    初代 + 政權二世：挫蛋先鋒(二世)
    復國二代 + 政權一世：挫蛋軍政府II
    復國二代 + 政權二世：挫蛋軍政府II(二世)
    """
    root = str(regime_name_root or "").strip()
    country_generation = max(1, int(country_generation or 1))
    regime_generation = max(1, int(regime_generation or 1))

    display = root

    # 國家「亡國後復國」才增加羅馬數字；第一世代不顯示 I。
    if country_generation > 1:
        display += to_roman(country_generation)

    # 同一政權合法交接才增加括號世系；第一世不顯示。
...
def _advance_country_rebirth_identity(country_key, data):
    ...
    new_country_gen = current_country_gen + 1

    data["country_generation"] = new_country_gen
    # gen 保留作為相容欄位，但 V6 起只代表「亡國復國世代」。
    data["gen"] = new_country_gen
...
def _ensure_dynamic_country_state(country_key, data):
    ...
    # V6：把「亡國復國世代」與「同一政權合法交接世系」完全拆開。
    if "country_generation" not in data:
        data["country_generation"] = (
            _infer_country_generation_from_history(data)
        )
    data["country_generation"] = max(
        1,
        int(data.get("country_generation", 1) or 1),
    )
    # 舊欄位 gen 保留供既有 UI / 存檔相容，但新規則只在亡國重建時增加。
