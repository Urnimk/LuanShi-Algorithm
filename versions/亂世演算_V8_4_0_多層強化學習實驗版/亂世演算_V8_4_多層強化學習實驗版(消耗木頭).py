...
def update_rts_economy_and_jobs(data):
    ...
    if birth_policy_turns > 0:
        ...
        data["farmers"] += random.randint(8,15)
...
def select_international_dread_coalition_target(countries, alive_sorted):
    ...
    selected = [k for k in candidates if number < INTERNATIONAL_DREAD_SELECTION_CHANCE]

    # temp = max(selected, key=lambda k: int(countries[k].get("international_dread", 0) or 0))
    # selected = random.choice(candidates)

    # print(
    #             f"[國際恐慌] 【{candidates} 忌憚值滿200000！{number} {selected} 被選中"
    #         )

    # if len(candidates) > 0 and number < INTERNATIONAL_DREAD_SELECTION_CHANCE :
    #     print(
    #         f"[國際恐慌] 【{candidates} 忌憚值滿200000！{number} {selected} 被選中"
    #     )
