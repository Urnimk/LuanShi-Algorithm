            ...
            for attacker_key in active_countries:
                ...
                if action == 0:
                    ...
                    if possible_defenders:
                        ...
                        if (
                            def_entity.get("type") == "ALLIANCE"
                            and random.random()
                            < float(
                                countries[defender_key].get(
                                    "world_single_war_chance", 0.0
                                )
                                or 0.0
                            )
                            ...
                            def_entity = {
                                "type": "SINGLE",
                                "name": countries[defender_key].get(
                                    "display_name", defender_key
                                ),
                                "members": [defender_key],
                                "power": countries[defender_key].get("power", 0),
                            }
                ...
                # ---------------------------------------------------- 
                    ...
                    if not c_data["current_alliance"]:  # 僅在尚未加入同盟時執行
                        ...
                        if existing_alliances and random.random() < 0.5:
                            ...
                            join_chance = 0.72 - min(
                                0.42,
                                c_data.get("infamy", 0) / 180.0,
                            )
                            join_chance *= float(
                                c_data.get("world_alliance_bias", 1.0) or 1.0
                            )
                            join_chance = float(
                                _clamp(join_chance, 0.12, 0.95)
                            )
