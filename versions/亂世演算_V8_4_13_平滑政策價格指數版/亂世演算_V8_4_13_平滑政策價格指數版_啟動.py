...
try:
    from 亂世演算_V8_4_13_平滑政策價格指數版 import (
        thread_stat_monitor,
    ...
    pattern = os.path.join(
        ...
        "亂世演算_V8_4_13_平滑政策價格指數版*.py",
    )
    candidates = sorted(glob.glob(pattern), key=os.path.getmtime, reverse=True)
    if not candidates:
        raise
    spec = importlib.util.spec_from_file_location("亂世演算_v8413_engine", candidates[0])
    ...
    def refresh_world_market_panel(self):
        ...
        self.market_bank_label.configure(
            text=(
                ...
                f"政策指數 {float(bank.get('policy_price_index', 1.0) or 1.0):.3f}｜"
