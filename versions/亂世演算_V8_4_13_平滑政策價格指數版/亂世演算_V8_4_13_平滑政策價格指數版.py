...
from policy_index import policy_cost, initialize as policy_index_initialize, settle_season as policy_index_settle_season
...
ENGINE_VERSION = "亂世演算V8_4_13_平滑政策價格指數版"
SEASON_SECONDS = 1000
SEASON_YEAR = 2 # 幾秒/年 
SEASON_RATIO = 1 # 加速幾倍
...
def _world_bank_grant(bank, country_data, amount, reason="援助"):
    ...
    bank["last_action"] = f"{reason}：{amount} 大洋"
    return amount
...
def _world_bank_pay_bounty(bank, country_data, amount, reason="國際懸賞"):
    ...
    bank["last_action"] = f"{reason}：{paid} 大洋"
    return paid
...
def _world_bank_recirculate_liquidity(bank, countries, turn_counter):
    ...
    if distributed:
        ...
        bank["last_action"] = f"逆週期流動性回流：{distributed} 大洋／{len(grants)} 國"
        print(f"🏦 [流動性回流] 世界銀行從既有庫存釋出 {distributed} 大洋，支援 {len(grants)} 個現金短缺國家。")
    return grants
...
def _world_bank_issue_for_market(bank, countries, market, resource, turn_counter):
    ...
    for order in sells:
        ...
        qty = min(int(order.get("qty", 0) or 0), available)
        if qty <= 0:
            continue
        gross = int(round(qty * seed))
        if gross <= 0:
            continue
        # 發行額正好等於付款額，沒有從原有庫存護盤，也不留下未使用新幣。
        bank["issued_total"] = int(bank.get("issued_total", 0) or 0) + gross
        bank["market_issued_total"] = int(bank.get("market_issued_total", 0) or 0) + gross
    ...
    if purchases:
        ...
        bank["last_action"] = f"市場發行：新增 {total} {currency}，按預設價購買 {quantity} {MARKET_RESOURCE_META[resource]['name']}"
        ...
        print(f"🏦 [市場發行] {MARKET_RESOURCE_META[resource]['name']}觸及低點；世界銀行新增 {total} {currency}，以每單位 {seed:.2f} 購買 {quantity} 單位。")
    return purchases
...
def _market_match_orders(countries, market, bank, turn_counter):
    ...
    for resource in MARKET_RESOURCES:
        while True:
            ...
            print(
                f"💱 [世界市場成交] 【{trade['buyer_name']}】以 {gross} 大洋向"
                f"【{trade['seller_name']}】買入 {qty} {trade['resource_name']}，"
                f"成交單價 {unit_price:.2f}，世界銀行手續費 {fee}。"
            )

    # 一次通縮事件只換一次名稱；改名不改 gold、價格或購買力。
    depressed = any(
        float(market["stats"][res].get("last_price", MARKET_RESOURCE_META[res]["seed_price"]))
        <= MARKET_RESOURCE_META[res]["seed_price"] * WORLD_BANK_ISSUE_TRIGGER_RATE
        for res in MARKET_RESOURCES
    )
    if not depressed:
        bank["deflation_episode_active"] = False
    if depressed and not bank.get("deflation_episode_active", False):
        bank["deflation_episode_active"] = True
        # 開局第一次為大洋；下次獨立通縮事件才使用下一個備用名稱。
        if int(bank.get("market_issue_operations", 0) or 0) > 0:
            index = (int(bank.get("currency_name_index", 0) or 0) + 1) % len(WORLD_CURRENCY_NAME_OPTIONS)
            bank["currency_name_index"] = index
            bank["currency"] = WORLD_CURRENCY_NAME_OPTIONS[index]
            bank["currency_rename_last_turn"] = int(turn_counter)
            print(f"💱 [貨幣名稱輪替] 再次出現市場通縮，貨幣名稱改為 {bank['currency']}；幣值與帳面金額不變。")
...
def thread_stat_monitor():
    ...
    while True:
          ...
          f"（分工比例 糧:{p_food}% / 金:{p_metal}% / 木:{p_wood}%）")
                ...
                # ----------------------------------------------------
                    ...
                    if old_infamy > 0 and gold_stock >= policy_cost(60 * MONEY_RATIO, world_bank):
                        ...
                        print(
                            f"🤝 [外交修復] 【{c_data['display_name']}】支付 {diplomacy_cost} 大洋進行外交斡旋，"
                ...
                # ----------------------------------------------------
                    ...
                    if action_success:
                        ...
                        print(
                            f"👶 [鼓勵生育] 【{c_data['display_name']}】投入 {spent} 大洋，"
                ...
                # ----------------------------------------------------
                    ...
                    if action_success:
                        ...
                        print(
                            f"🏭 [增產補貼] 【{c_data['display_name']}】投入 {spent} 大洋，"
        ...
        rank_text = f"===== 亂世演算 第 {count_round} 屆 戰力大戰 ===== 剩餘 {int(time_total - (time.time() - start_time))} 年結束\n"
        rank_text += (
        ...
        if (time.time() - start_time) > time_total:
            ...
            new_policy_index = policy_index_settle_season(
                world_bank, countries, world_market, MARKET_RESOURCE_META, count_round
            )
